{{--
    Rincian satu fasilitas SP, yaitu bangunan dan sarana tetap milik satuan
    permukiman (agents/rules.md bagian 4b).

    Dibedakan dari inventaris: fasilitas adalah bangunan tempat warga
    berkegiatan, sedangkan inventaris adalah barang bergerak. Perbedaan itu
    terbaca dari kolomnya, sebab fasilitas memiliki titik koordinat sedangkan
    inventaris tidak.

    Halaman ini dibuat 2026-08-19 bersama tautan objek pengaduan. Sebelumnya
    fasilitas hanya memiliki halaman daftar, sehingga keluhan warga atas
    sebuah bangunan tidak punya tempat untuk ditampilkan kembali.
--}}
@extends('layouts.app')
...
@section('content')
    ...
    <x-sim.page-header :judul="$data['nama_fasilitas']"
        ...
        <x-slot:aksi>
            @if ($bolehUbah)
                <button type="button"
                    @click="$dispatch('buka-modal', 'formUbahFasilitas')"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <dl class="mt-5 space-y-3 border-t border-gray-200 pt-5 text-theme-sm dark:border-gray-800">
                    <div class="flex justify-between gap-3">
                        <dt class="text-gray-500 dark:text-gray-400">Jenis fasilitas</dt>
        ...
        <div x-data="hashTabs('rincian')" class="min-w-0">
            <div class="min-w-0 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex gap-1 overflow-x-auto no-scrollbar border-b border-gray-200 px-2 pt-2 dark:border-gray-800"
                    role="tablist" aria-label="Rincian fasilitas SP">
                    @foreach ([
                        'rincian' => 'Rincian Fasilitas',
                        'log' => 'Catatan Log',
                    ] as $kunci => $label)
                ...
                <div x-show="tab === 'rincian'" role="tabpanel" class="p-5 sm:p-6">
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-theme-xs text-gray-500 dark:text-gray-400">Nama fasilitas</dt>
                        ...
                        <div>
                            <dt class="text-theme-xs text-gray-500 dark:text-gray-400">Titik lokasi</dt>
                            <dd class="mt-0.5 text-theme-sm tabular-nums text-gray-800 dark:text-white/90">
                                @if (! empty($data['lintang']))
                                    ...
                                    <x-sim.tautan-peta class="mt-1.5" :lintang="$data['lintang']"
                                        :bujur="$data['bujur']" :label="$data['nama_fasilitas']" />
                                @else
                                    -
                                @endif
                            </dd>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <dd class="mt-0.5 space-y-1 text-theme-sm text-gray-800 dark:text-white/90">
                                    {{-- Jamak sejak Putaran 14; satu bangunan beberapa sisi. --}}
                                    @forelse ($berkasFoto as $b)
                                        <x-sim.tautan-dokumen modul="fasilitas_sp"
                                            :id="$data['id_fasilitas_sp']" :berkas="$b['nama_file']" />
                                    @empty
                                        <x-sim.tautan-dokumen modul="fasilitas_sp"
                                            :id="$data['id_fasilitas_sp']" :berkas="null" />
                                    @endforelse
                                </dd>
                            </div>
                            <div>
                ...
                <div x-show="tab === 'log'" x-cloak role="tabpanel">
                    <x-sim.catatan-log nama-tabel="fasilitas_sp" :record-id="$data['id_fasilitas_sp']" />
                </div>
            </div>
        </div>
    </div>
    ...
    @if ($bolehUbah)
        <x-sim.modal-form nama="formUbahFasilitas" judul="Ubah Data Fasilitas"
            keterangan="Perubahan tercatat pada audit log."
            :aksi="route('fasilitas.perbarui', $data['id_fasilitas_sp'])" metode="PUT" ukuran="lg"
            label-simpan="Simpan Perubahan">
            @include('pages.sp.form-fasilitas', ['data' => $data, 'awalan' => 'ubah'])
        </x-sim.modal-form>
    @endif
@endsection
