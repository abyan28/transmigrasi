        ...
        <div x-data="hashTabs('biodata')" class="min-w-0">
            <div class="min-w-0 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div x-show="tab === 'dokumen'" x-cloak role="tabpanel">
                    {{--
                        Sejak Putaran 12 berkas transmigran tinggal di registri
                        `berkas` lewat pivot, bukan lagi satu kolom
                        `dokumen_pendukung`. Panel ini dulu membaca kolom itu,
                        sehingga KTP, KK, dan SK yang sudah terunggah tidak
                        muncul sama sekali: unggahan tanpa jalan dibuka (R-26).
                    --}}
                    @if (empty($berkasKeluarga))
                    ...
                    @else
                        <ul class="space-y-3 p-5 sm:p-6">
                            @foreach ($berkasKeluarga as $b)
                                <li>
                                    <x-sim.tautan-dokumen modul="transmigran" :id="$data['id_transmigran']"
                                        :berkas="$b['nama_file']" />
