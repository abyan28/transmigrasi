...
@section('content')
    ...
    @php
        $persenBelumSelesai = $total > 0 ? round(($belumSelesai / $total) * 100, 1) : null;
    @endphp

    {{-- Wadah Ringkasan dengan Pilihan Tampilan (Bilah Ramping vs Kartu Lama) --}}
    <div x-data="{
            modeRingkas: localStorage.getItem('pref_ringkasan_pengaduan') !== 'kartu',
            toggleMode() {
                this.modeRingkas = !this.modeRingkas;
                localStorage.setItem('pref_ringkasan_pengaduan', this.modeRingkas ? 'strip' : 'kartu');
            }
        }"
        class="mb-6">
        ...
        <div class="mb-2 flex items-center justify-between gap-2 px-1">
            <div class="text-theme-xs text-gray-500 dark:text-gray-400">
                <p class="font-medium">Ringkasan Penanganan Pengaduan</p>
        ...
        <div x-show="modeRingkas" x-transition.opacity>
            <x-sim.metric-strip>
                <x-sim.metric-item label="Total Pengaduan" :nilai="number_format($total, 0, ',', '.')"
                    satuan="laporan" ikon="pengaduan" warna="teal" keterangan="Warga & petugas" />
                <x-sim.metric-item label="Belum Selesai" :nilai="number_format($belumSelesai, 0, ',', '.')"
                    satuan="laporan" ikon="pengaduan" warna="navy" :prosentase="$persenBelumSelesai" />
                <x-sim.metric-item label="Menunggu Diterima" :nilai="number_format($menungguDiterima, 0, ',', '.')"
                    satuan="laporan" ikon="perhatian" warna="gold" keterangan="Perlu disaring petugas"
                    :perhatian="$menungguDiterima > 0" />
                <x-sim.metric-item label="Prioritas Mendesak" :nilai="number_format($mendesak, 0, ',', '.')"
                    satuan="laporan" ikon="mendesak" warna="red" keterangan="Tindak lanjut segera"
                    :mendesak="$mendesak > 0" />
            </x-sim.metric-strip>
        </div>
