...
- [✓] Task 2.31 - Modul Pengelolaan Konten (CMS) 5 tab `[Sulit]` (Selesai 2026-09-01)
...
- [✓] Task 2.32 - Halaman Panduan Penggunaan `[Sedang]` (Selesai 2026-08-31)
...
- [✓] Task 2.33 - Halaman Tentang Sistem `[Mudah]` (Selesai 2026-08-31)
  * `/tentang`, 21 KB. Profil sistem dan kawasan Kobalima Timur
  * Sebelumnya tidak tercatat sama sekali
- [✓] Task 2.34 - Pengaturan Penilaian Kondisi SP `[Sedang]` (Selesai 2026-08-21)
...
- [✓] Task 4.1 - CRUD wilayah bertingkat + seeder provinsi/kabupaten `[Sedang]` -- **SELESAI 2026-09-03**. `WilayahSeeder` (38 provinsi + 514 kabupaten, id = kode BPS; kecamatan/desa lokus) + `WilayahController` + `/wilayah` beralih penuh ke Eloquent. Tiga temuan: alamat `/wilayah/{id}` ambigu -> `/wilayah/{tingkat}/{id}`, `UppercaseInput` merusak `tingkat`, dan suite Feature perlu `RefreshDatabase` + seeder per-kelas. Rincian: `session-notes.md` blok Task 4.1
...
- [✓] Task 4.5 - Data master satuan + faktor konversi ke ton `[Mudah]` -- **SELESAI 2026-09-03**. `SatuanSeeder` (Ton/Kuintal/Kilogram) + `MasterSatuanController`; `faktor_ke_ton` divalidasi `gt:0` sebab faktor nol membuat volume panen lenyap dari rekap. Hapus ditolak bila masih dipakai komoditas. 11 uji
...
- [✓] Task 4.2 - CRUD satuan permukiman (SP) beserta koordinat `[Sedang]` -- **SELESAI 2026-09-03**. `SpSeeder` (6 SP lokus, `desa_id` diturunkan dari nama) + `SpController`; penjaga rentang min/maks keadaan wilayah (5 pasangan + pH) sebab rentang terbalik terbaca sebagai rentang kosong pada Monografi SP. Hapus diperiksa terhadap 7 relasi turunan. 8 uji
...
- [✓] Task 4.8 - CRUD parameter & ambang Penilaian Kondisi SP `[Sedang]` -- **SELESAI 2026-09-03**. `PenilaianKondisiController` + `PenilaianKondisiSeeder`; penjaga ambang WAJIB menurun (ambang Mandiri < Berkembang membuat Berkembang mustahil dicapai) + tolak bobot desimal yang dibulatkan diam-diam oleh TINYINT. 8 uji
...
- [✓] Task 5.1 - Peralihan transmigran + `anggota_keluarga` ke Eloquent `[Sedang]` ✅ SELESAI 2026-09-04
  ...
  * HASIL: `TransmigranController` (index + detail; private `daftar()`/`baris()`/`barisAnggota()` -> larik ber-kunci PERSIS `DummyData::transmigran()`/`anggotaKeluarga()`). `TransmigranSeeder` (transmigran `firstOrNew`+`uuid ??=` supaya uuid stabil; anggota `updateOrCreate`) didaftarkan di `DataMasterSeeder` + `DatabaseSeeder` setelah `BerkasSeeder`. 2 closure `routes/internal.php` -> controller (nama + `->where('id','[0-9]+')` tetap). **Lingkup minimal**: rute tulis + `rumah`/`lahan`/`berkas*`/`riwayatKk`/`calonPengganti`/data poktan pada rincian MASIH `DummyData` (pindah Task 5.2/5.3/6). `ViewServiceProvider` tak disentuh. `tests/Database/TransmigranTest.php` +8. Verifikasi: Feature 732, Database 303, pint bersih, banding-skema NOL SELISIH, tautan-statis 14
- [✓] Task 5.2 - CRUD transmigran + unggah KTP/KK/SK terpisah `[Sulit]` ✅ SELESAI 2026-09-04
  ...
  * HASIL: `TransmigranController` +`simpan`/`perbarui`/`hapus`/`catatPeristiwa`/`gantiKepalaKeluarga` (semua transaksi). `perbarui` sinkron anggota id-based (baris ber-id update, tanpa id create, Aktif yang hilang -> soft delete) HANYA bila `_anggota_disunting` hadir -- modal ubah-per-baris di halaman daftar tak memuat anggota. Suksesi: rekam `riwayat_kepala_keluarga` (2 sisi identitas) + sunting transmigran (nama/nik/no_kk + biodata pengganti) + soft-delete baris anggota pengganti; `nasib_ketua_poktan` DIVALIDASI (required bila `DummyData::poktanDiketuaiKeluarga` tak kosong) tapi mutasi `poktan` DITANGGUHKAN ke Task 6 (poktan masih DummyData)
  * `detail()`: `berkasKtp/Kk/Sk/Keluarga` -> `$t->berkas` (map pivot.peran), `riwayatKk` -> `$t->riwayatKepalaKeluarga`, `calonPengganti` -> turunan Eloquent (pasangan lalu usia desc)
  * `form.blade.php`: repeater +hidden `[id]` per baris + Alpine `anggota` map/`tambahAnggota` +`id`; +hidden `_anggota_disunting` di langkah 3 (kecuali `awalan=ubahBaris`)
  * `UppercaseInput::$kecualikan` +9 pemilih enum (`jenis_kelamin`/`agama`/`pendidikan_terakhir`/`status_tinggal`/`hubungan`/`hubungan_pengganti`/`kegiatan`/`status`/`nasib_ketua_poktan`)
...
- [✓] Task 6.4 - CRUD profil poktan dan data ketua `[Sedang]` ✅ SELESAI 2026-09-04
- [✓] Task 6.5 - CRUD daftar anggota poktan + status keaktifan `[Sedang]` ✅ SELESAI 2026-09-04 (bersama 6.4)
  * HASIL: `PoktanController` (index/detail/simpan/perbarui/hapus) + `AnggotaPoktanController`
    (simpan/perbarui, TANPA hapus). Ketua 3 jalur (`asal_ketua`): validasi bersyarat
    `Rule::requiredIf`; `pisahkan()` membersihkan kolom cabang tersembunyi. `slug`
    otomatis (`BerslugOtomatis`). SK = FK TUNGGAL `berkas_id` (bukan pivot) lewat
    `MenyimpanBerkas::rekamBerkas`. Langkah 3 form = repeater anggota (create-only,
    lewati yang sudah anggota; penanda `_anggota_disunting`).
  * `AnggotaPoktanController`: `asal_wakil` 2 jalur; status lifecycle (`Sudah Keluar`
    wajib `tanggal_keluar`); aturan "satu keluarga hanya Aktif di SATU poktan"
    (`rules.md` 6.4) divalidasi manual (`ValidationException`). Anggota TAK PERNAH
    dihapus -> tak ada `anggota-poktan.hapus`.
  * `identitasKetua()` (3 jalur) + `identitasWakilAnggota()` (2 jalur) + `kekuatanPoktan()`
    (jumlah_anggota + luas kelompok, semua diturunkan). `App\Support\RekapLahan::keluarga(?Lahan)`
    dipakai bersama controller + `ViewServiceProvider::petaKeluarga()` (kini Eloquent).
    `anggotaPoktanPerPoktan` composer -> Eloquent.
  * `PoktanSeeder` baru (poktan + anggota_poktan, `berkas_id` bila berkasnya ada),
...
**Masih menunggu:**
- ~~**Isian unggah pada Form Lahan tidak punya tempat penyimpanan (ditemukan Putaran 14).**~~ **SELESAI Putaran 15, pilihan (a):** isian `jenis_dokumen` dan `file_dokumen` dicabut dari form lahan; langkah 3 kini bacaan SHM/HPL beserta tautan ke Data Transmigran dan Data Kawasan. Berkas lahan memang sudah pindah ke kawasan (HPL) dan transmigran (SHM) sejak Putaran 12.
...
- ~~Konfirmasi apakah lahan pekarangan bisa lebih dari satu per KK~~ **TERJAWAB 2026-08-18: tidak.** Satu transmigran menerima satu lahan pekarangan dan satu lahan usaha (`rules.md` 7.8). **Sejak Putaran 15 relasinya one-to-one** (`UNIQUE (transmigran_id)`): kedua bidang jadi kolom pada satu baris per KK.
