...
**Rute dokumen polos** `/laporan/{slug}/dokumen` (`laporan.dokumen`) memakai `layouts/dokumen` — tanpa sidebar/header. **Tabelnya** sama persis dengan halaman berbingkai (`@include('pages.laporan.isi.{slug}')`), TETAPI sejak Putaran 5 ia **dokumen resmi**, bukan sekadar kertas polos:

- **Kop surat `<x-sim.kop-laporan :slug>`** (flex/grid, TANPA elemen tabel — penjaga `kolomTerlebarDariHtml`): dua lambang (logo Kementerian Transmigrasi kiri, lambang Kabupaten Malaka kanan), teks instansi dari `LaporanData::instansi()` (kabupaten/provinsi dari `DummyData::kawasan()`; telepon/email PLACEHOLDER dari `refs/`; istilah "Email" bukan "Surel", §10.1), lalu blok judul di tengah: `LAPORAN <judul>`, sub-judul opsional (`meta['subjudulDokumen']`), baris **"TAHUN ..."** (getter Alpine `tahunDokumen`: rentang bila filter rentang tahun aktif; satu tahun bila pemilih tahun tunggal; selain itu `LaporanData::tahunDokumenBawaan()` = tahun terakhir `deretTahunan()`, bukan `date('Y')`), lalu kalimat cakupan (`x-text="kalimatCakupan"`, wajib tetap tercetak — §12 poin 8), dasar periode, catatan.
- **TANPA bilah filter** (`<x-sim.filter-laporan>` hanya `! $dokumen`). Cakupan `x-data="filterLaporan(...)"` tetap pada `<div>` pembungkus supaya `dariHash()` menerapkan filter yang dibawa hash; baris tersembunyi lewat `x-show` (`display:none`).
- Bingkai kartu (`rounded-2xl shadow-sm`) dilepas pada rute dokumen.

**Tombol "Generate Laporan"** (halaman berbingkai, gaya primer, `target=_blank rel=noopener` + sr-only) menggantikan "Buka di tab baru". `:href="@js(route('laporan.dokumen',$slug)) + hashFilter"` — `hashFilter` (getter `filterLaporan`) menserialisasi keadaan filter aktif ke fragmen `#sp=..&td=..&ts=..&th=..&<dimensi>=..` (via `URLSearchParams`). GitHub Pages tidak melayani query string tetapi hash murni sisi peramban; tak menambah rute (`sim:tautan-statis` tetap 222). `x-data` PINDAH dari `<article>` ke `<div>` pembungkus supaya tombol di dalam `page-header` (di luar `<article>`) ikut membaca keadaan filter.
...
**Isi bilah per laporan** dari `LaporanData::filterLaporan($slug)`: Satuan Permukiman (daftar master, bukan cacahan baris — §19a) + `tahun` (sepasang rentang) + `dimensi` khas (status, komoditas, jenis, poktan). Larik kosong = laporan belum berfilter, kerangka tak merender bilah. Terpasang (**ketujuh laporan**, D3 selesai 2026-08-29): **Transmigran** (SP + Tahun Kedatangan + Status Tinggal), **Poktan** (SP — satu tabel per poktan), **Alsintan** (SP + Tahun Pengadaan + Jenis Alat — grup per SP + subtotal `x-text`), **Saprotan** (SP + Tahun Pengadaan + Komoditas + Jenis — dua tabel datar), **Hasil Panen** (SP + Tahun Anggaran Bantuan §16a + Komoditas — grup per SP, produktivitas via `rasioTampak`), **Monografi SP** (SP — potret per SP: baris ikhtisar + `<section>` per SP (Pendahuluan sampai Sosial Budaya)), **Rekap Indikator Kawasan** (SP — tabel per SP menyempit; 4 blok ringkasan kawasan TIDAK menyempit sebab dari dashboard, dengan catatan kejujuran `x-show="adaFilter"`).

**Atribut data kosong (`''`) = dimensi tak berlaku atas baris itu.** `cocok()` melewati dimensi yang atributnya kosong/absen — satu bilah dua-dimensi (Komoditas + Jenis) melayani dua tabel Saprotan tanpa saling mengganggu.

**Granularitas penyaringan mengikuti struktur laporan.** Laporan berbaris datar (Transmigran) menyaring `<tr>`. Laporan satu-tabel-per-entitas (Poktan) menyaring wadah tabel. Laporan grup-per-SP dengan subtotal (Alsintan D3-3a; Saprotan/Hasil Panen menyusul) menyaring baris, menyembunyikan baris grup-header + subtotal saat grupnya kosong (`x-show="! kosong(tabel, selSp(id))"`), dan menghitung ulang subtotal & total lewat `x-text="jumlahTampak(tabel, kolom, desimal, selSp(id))"` (produktivitas tertimbang: `rasioTampak(tabel, pembilang, penyebut)`). Angka Blade dipertahankan sebagai jaring bila JS mati. `rules.md` §8o: baris total yang menyempit menyatakan cakupan aktif.

Helper Alpine (`resources/js/filter-laporan.js`): `cocok(tr)`, `kosong`/`cacahTampak`/`jumlahTampak`/`rasioTampak` (menerima elemen ATAU NodeList/array via `_baris()`, plus param `penanda`), `selSp(id)`, `kalimatCakupan`, `bersihkan()`.
...
### 6.12 Laporan Monografi SP diperluas + "Catat Peristiwa" anggota keluarga (Putaran 6, 2026-08-29)

**Laporan Monografi SP** kini bukan hanya Keadaan Wilayah. Tiap `<section data-baris data-sp>` per SP memuat, berurutan: **Pendahuluan** (kalimat + `<dl>` ringkas), **Keadaan Wilayah** (judul lama "Bab II. Keadaan Wilayah" → "Keadaan Wilayah"; `<h2>` halaman → "Monografi per Satuan Permukiman"), **Kependudukan** (penempatan per daerah asal, keadaan penduduk sekarang [`x-text="nilaiKependudukan(spId, kunci)"` ikut tahun terpilih], struktur umur, usia sekolah, mutasi penduduk), **Sosial Ekonomi** (lahan, sertifikat, tanaman pangan, infrastruktur), **Sosial Budaya** (pendidikan, kesehatan, agama + rumah ibadah, olahraga, keamanan, alsintan, inventaris, fasilitas umum). **Judul sub-bagian tanpa awalan "Bab X."**

Tiap sub-tabel dirender oleh partial `pages/laporan/isi/_tabel-dok.blade.php` dari deskriptor `LaporanData::tabelDok()` (`judul`, `kolom`, `baris`, `total`, `catatan`, `kosong`) — `.tabel-dokumen` + `<caption>` anak pertama + `overflow-x-auto` + `motif-baris-total`; tabel kosong menampilkan "belum ada data" (pemilik proyek memilih "tampilkan apa adanya + catatan"). Baris `_tabel-dok` memakai `<tr class=...>` biasa (bukan `data-baris`) sehingga tidak ikut hitungan penjaga uji; ia tampil/sembunyi bersama `<section>` induknya. Struktur umur & mutasi = **angka contoh turunan** (`DummyData::strukturUmurSp`/`mutasiPendudukSp`), ditandai di catatan. Getter JS baru: `nilaiKependudukan(spId, kunci)` baca `konfig.kependudukanTahun[spId][tahun]`, `labelTahunTerpilih()`.
