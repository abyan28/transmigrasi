...
**Isi bilah per laporan** dari `LaporanData::filterLaporan($slug)`: Satuan Permukiman (daftar master, bukan cacahan baris — §19a) + `tahun` (sepasang rentang) + `dimensi` khas (status, komoditas, jenis, poktan). Larik kosong = laporan belum berfilter, kerangka tak merender bilah. Terpasang: **Transmigran** (D3-1: SP + Tahun Kedatangan + Status Tinggal), **Poktan** (D3-2: SP saja — satu tabel per poktan), **Alsintan** (D3-3a: SP + Tahun Pengadaan + Jenis Alat — grup per SP + subtotal `x-text`), **Saprotan** (D3-3b: SP + Tahun Pengadaan + Komoditas Benih + Jenis Sarana — dua tabel datar). Menyusul Hasil Panen, Rekap Indikator Kawasan, Monografi SP.
