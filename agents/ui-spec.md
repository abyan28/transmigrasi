...
**Isi bilah per laporan** dari `LaporanData::filterLaporan($slug)`: Satuan Permukiman (daftar master, bukan cacahan baris — §19a) + `tahun` (sepasang rentang) + `dimensi` khas (status, komoditas, jenis, poktan). Larik kosong = laporan belum berfilter, kerangka tak merender bilah. Terpasang (enam dari tujuh): **Transmigran** (D3-1: SP + Tahun Kedatangan + Status Tinggal), **Poktan** (D3-2: SP — satu tabel per poktan), **Alsintan** (D3-3a: SP + Tahun Pengadaan + Jenis Alat — grup per SP + subtotal `x-text`), **Saprotan** (D3-3b: SP + Tahun Pengadaan + Komoditas + Jenis — dua tabel datar), **Hasil Panen** (D3-3c: SP + Tahun Anggaran Bantuan §16a + Komoditas — grup per SP, produktivitas via `rasioTampak`), **Monografi SP** (D3-5: SP — potret per SP, menyembunyikan baris ikhtisar + `<section>` Bab II). Menyusul: **Rekap Indikator Kawasan** (D3-4, perlu agregasi 16 indikator per SP lebih dulu).
