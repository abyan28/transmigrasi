...
**Granularitas penyaringan mengikuti struktur laporan.** Laporan berbaris datar (Transmigran) menyaring `<tr>`. Laporan satu-tabel-per-entitas (Poktan) menyaring wadah tabel. Laporan grup-per-SP dengan subtotal (Alsintan/Saprotan/Hasil Panen, D3-3) menyaring baris DAN menghitung ulang subtotal lewat `jumlahTampak()`. Helper `kosong(cakupanEl, penanda)` melayani pesan "tidak ada yang cocok" untuk kedua granularitas.
