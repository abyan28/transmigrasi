...
**Isi bilah per laporan** dari `LaporanData::filterLaporan($slug)`: Satuan Permukiman (daftar master, bukan cacahan baris — §19a) + `tahun` (sepasang rentang) + `dimensi` khas (status, komoditas, jenis, poktan). Larik kosong = laporan belum berfilter, kerangka tak merender bilah. Terpasang: **Transmigran** (D3-1: SP + Tahun Kedatangan + Status Tinggal), **Poktan** (D3-2: SP saja — satu tabel per poktan, `x-show="cocok($el)"` pada wadah `<div data-poktan-wadah data-sp>`, bukan baris). Menyusul D3-3..D3-5.
