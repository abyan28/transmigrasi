...
## 1n. Revisi Putaran 2: Menu Laporan + Filter Rentang Tahun (2026-08-28)

Rombongan A butir 3, 4, 5 dari bagian 6. Kerangkanya saja; isi kolom tiap laporan (Tahap 2c) menunggu format dari dinas.

### 1n.1 Keputusan 2026-08-17 dibalik: laporan adalah dokumen bernama

`rules.md` §12 dahulu (2026-08-17) menempelkan tombol ekspor pada tiap tabel dan menghapus halaman `/laporan` terpusat. Alasannya waktu itu masuk akal: halaman terpusat menawarkan sembilan unduhan tanpa satu pun kontrol filter.

Yang tidak terlihat waktu itu: tombol ekspor berada di dalam kerangka bersama `halaman-daftar`, sehingga **muncul otomatis di belasan halaman daftar** — sebagian besar tanpa laporan di baliknya. Itu kontrol mati (R-26), dan tak seorang pun mengkliknya karena memang tidak menghasilkan apa-apa.

Berkas rujukan di `refs/` menyelesaikan perdebatan: "Lap. Akhir Panen Jagung Polri MT. I 2025", "LAPORAN MONOGRAFI UPT KAPITAN MEO 2025", "laporan alsintan", "laporan saprotan" — semuanya **dokumen berformat tetap** dengan kolom baku yang ditentukan dinas, bukan potret tabel modul mana pun yang sedang tersaring.

Keputusan: menu **"Laporan"** jadi rumahnya, tiap laporan satu halaman. Tombol ekspor dicabut dari `halaman-daftar` dan dari 9 pemasangan eksplisit; komponen `tombol-ekspor` dihapus. `§12` ditulis ulang menyeluruh (poin 5 sampai 14).

### 1n.2 Tujuh halaman laporan kerangka

Laporan Hasil Panen, Laporan Monografi SP, **Laporan Alsintan**, **Laporan Saprotan** (dua laporan terpisah mengikuti dua berkas rujukan, bukan digabung), Rekap Indikator Kawasan, Laporan Daftar Poktan, Laporan Daftar Transmigran (memuat data Rumah dan Lahan tiap transmigran).

Komponen `x-sim.kerangka-laporan` memberi tiap halaman: judul dokumen, **pernyataan cakupan sebagai teks** (wilayah + dasar periode) bukan kontrol filter (§12 poin 8), penampung tabel berlabel "Format kolom sedang disusun", dan tombol unduh jujur "segera hadir" (R-26, pembangkitan berkas Tahap 10).

Halaman laporan **tidak punya penyaring sendiri**. Rencana pewarisan filter lewat pintasan dari halaman daftar pasangan ditunda ("menu Laporan dulu", keputusan pemilik proyek).

### 1n.3 Filter rentang tahun: hanya tiga halaman, bukan "tiap datatable"

Butir aslinya menyebut "tiap datatable". Setelah ditelaah, hanya **daftar transaksi bersumbu waktu** yang aman: `/panen`, `/penanaman`, `/audit-log`. Di sana tiap baris berdiri sendiri, sehingga menyaring rentang hanya menyempitkan daftar.

`/panen` dan `/penanaman` sudah punya penyaring tahun **tunggal**; diganti sepasang dari–sampai. `/audit-log` memperoleh penyaring tahun untuk pertama kalinya.

**Rekap agregat dikecualikan tegas.** `/panen/rekap` tetap tahun tunggal: menjumlahkan luas lintas tahun membuat bidang 2 ha yang ditanami tiga tahun terbaca 6 ha (§9 poin 8b). Ini **mengoreksi saran lisan 2026-08-27** yang sempat menyebut "rekap panen boleh dapat rentang tahun" — keliru.

Pemilik proyek juga melihat opsi `/kependudukan/rekap` tab tahun dan tabel Transmigrasi (`/transmigran` tahun kedatangan, `/rumah` tahun pembangunan) lalu tidak memilihnya. Cakupan sengaja sempit.

Penyaringannya dipusatkan di `DummyData::saringRentangTahun()` — dipakai ketiga rute — sehingga batas kosong, batas terbalik (ditukar, bukan mengosongkan daftar), dan baris tanpa tahun ditangani dengan cara yang sama di mana pun.

### 1n.4 Verifikasi

**635 uji hijau** (naik dari 623), `pint` tetap 31 berkas.

Penjaga baru:
- Tak ada `data-ekspor` di 17 halaman daftar; komponen `tombol-ekspor` benar-benar terhapus dan tak dirujuk berkas Blade mana pun.
- Menu "Laporan" ada, tiap submenunya menunjuk rute yang membalas 200 (R-24).
- Tiap halaman laporan memuat judul (`<title>`), "Cakupan laporan", "Dasar periode", "segera hadir", dan penanda "Data contoh".
- `saringRentangTahun` diuji pada batas kosong, terbalik, string kosong, dan tahun hilang.
- `/panen` dan `/penanaman` disaring rentang: tiap baris jatuh tepat ke satu rentang, tak ada yang hilang atau ganda.
- Komponen rentang tahun **tidak** dipakai di halaman rekap mana pun; `/panen/rekap` tetap `name="tahun"` tunggal.

Render nyata: `/laporan` + 7 halaman laporan, serta `/panen`, `/penanaman`, `/audit-log` dengan dan tanpa parameter rentang.
...
- [done 2026-09-02] **Utang Putaran 12 dibereskan, lahan menjadi satu baris per keluarga, dan penjaga isian yatim (Putaran 15).**
  ...
  * **Rencana saya sendiri keliru dua kali, dan keduanya sebab yang sama: ditulis dari ingatan, bukan dari dokumen.**
    - Form Lahan sempat saya sajikan sebagai "keputusan skema dengan tiga pilihan", padahal `session-notes.md` baris 369 sudah memutuskannya (`Lahan TIDAK dapat pivot`) dan `rules.md` 7.6a melarang unggahan di sana - aturan yang saya tulis sendiri pada Putaran 12. Dua dari tiga pilihan yang saya tawarkan MELANGGAR aturan yang sudah ada.
    - Daftar "9 domain berpivot sisanya" pada Putaran 14 juga salah; setelah `schema.sql` dibaca sungguhan, yang layak jamak hanya empat.
    Pelajarannya satu kalimat: **daftar kerja pun wajib diperiksa ke sumbernya, bukan diteruskan dari ingatan.**
  * **Audit ulang 15 keputusan Putaran 12 menemukan tiga yang cacat:**
    - **Keputusan 11 (`alsintan` induk dapat foto): NOL BARIS DIKERJAKAN**, padahal `tasklist.md` 981 sudah menuliskannya sebagai selesai. Dokumentasi mengklaim sesuatu yang tidak pernah ada.
    - **Keputusan 9 (`status_sertifikat`): CACAT SEBALIKNYA.** Kolomnya ada, datanya ada, tampil di tiga tempat (rincian transmigran, tab Legalitas lahan, Laporan Transmigran) - tetapi TIDAK ADA satu pun isian form untuk mengisinya. Kebalikan kontrol mati: data yang dibaca tetapi tidak dapat ditulis.
    - **Bangkai `dokumen_lahan`**: izin, enum `JenisDokumenLahan`, opsi referensi, dan tiga isian form masih hidup berhari-hari setelah tabelnya dicabut.
  * **Bangkai yang paling buruk bukan sekadar tidak tersimpan.** Isian jenis dokumen pada form lahan menyuruh petugas memilih HPL atau SHM PER BIDANG, padahal HPL adalah alas hak kawasan milik instansi dan SHM meliputi seluruh lahan satu keluarga. Ia bukan hanya mati, melainkan mengajarkan model data yang salah kepada pemakainya.
  * **Lahan menjadi SATU BARIS PER KELUARGA** atas keputusan pemilik proyek. Sebelumnya satu baris adalah satu BIDANG, sehingga keluarga dengan pekarangan dan lahan usaha menempati dua baris. Jumlahnya memang tetap - tepat satu pekarangan dan satu lahan usaha (7.8) - sehingga keduanya kini kolom, bukan baris. `UNIQUE (transmigran_id, peruntukan_lahan)` diganti `UNIQUE (transmigran_id)`, dan relasinya menjadi satu-ke-satu.
  * **Koordinat tetap DUA PASANG**, dan ini keputusan yang menentukan. Pekarangan dan lahan usaha berada di tempat yang berbeda, sehingga menyatukannya menjadi satu titik berarti membuang lokasi yang sudah terdata. Pilihan (a) dari tiga yang diajukan.
  * **`luas_pekarangan` NULL berarti BELUM MENERIMA, bukan menerima seluas nol.** Dua dari empat keluarga pada data contoh memang hanya memegang lahan usaha, dan keadaan itu sengaja dipertahankan agar tampilan yang membedakan keduanya benar-benar teruji. Sepola `status_sertifikat` 7.6c dan 10a.4c: pembaca tidak boleh dipaksa menebak beda "tidak ada" dari "belum didata".
  * **Rekap luas kini MENJUMLAH KOLOM, bukan baris.** Ini titik paling rawan pada putaran ini: melewatkan satu tempat membuat total luas mengecil tanpa memerahkan apa pun. Karena itu buktinya diangkakan - total tetap **6,0 ha** (0,5 pekarangan + 5,5 usaha) sebelum dan sesudah penggabungan, dan `kering + basah` tetap sama dengan `luas_usaha`.
  * **Penjaga isian yatim ditambahkan, dan inilah pencegah sesungguhnya.** Setiap `name=` pada berkas `pages/**/form*` wajib berpadanan di `schema.sql`, entah sebagai kolom tabel entah sebagai peran berkas pada registry. Putaran 12 memverifikasi SKEMA dengan impor MariaDB nyata tetapi tidak pernah membandingkan ISIAN FORM terhadap skema - itulah celah yang meloloskan tiga bangkai sekaligus.
  * **Penjaganya diuji dengan isian palsu**, bukan hanya ditulis: `name="kolom_palsu_xyz"` membuatnya merah, dicabut kembali hijau. Ia juga memeriksa dirinya sendiri (`expect($kolom)->toHaveKey(...)`) supaya tidak hijau palsu ketika pengumpulannya gagal, dan daftar kecualinya ditulis beserta alasan tiap butir - penjaga tanpa daftar kecuali akan dimatikan orang berikutnya begitu memerah sekali secara keliru.
  * **Dua Blade sempat rusak** oleh penyuntingan berbasis nomor baris yang meleset satu baris, mencabut `<div>` pembuka pada form fasilitas dan form kawasan. Ditangkap penjaga `menyeimbangkan tag HTML`, bukan oleh mata. Penyuntingan berbasis nomor baris pada berkas yang baru disunting menuntut pembacaan ulang lebih dulu.
  * **`jenis_dokumen_lahan` dicabut dari ENUM `referensi.jenis`.** Tertinggal setelah enum PHP, opsi referensi, dan rutenya sudah dicabut, dan tidak dipakai kode mana pun. Penjaga isian yatim tidak menangkapnya sebab ia memeriksa nama KOLOM, bukan nilai ENUM - batas yang perlu diketahui pembaca berikutnya. Diverifikasi: MariaDB kini menolak nilai itu dengan `Data truncated for column 'jenis'`.
  * **Verifikasi:** `schema.sql` diimpor ke MariaDB 10.4 sebagai basis data sekali-pakai - **61 tabel, 94 foreign key**, dan `UNIQUE` baru benar-benar menolak baris kedua dengan `Duplicate entry '1' for key 'uq_lahan_transmigran'`. Basis data uji dibersihkan sesudahnya; DB dev `sim_transmigrasi` tidak disentuh.
  * **Uji: 731 PASS / 7.409 assertions.** `pint` tetap 33 pre-existing. `sim:tautan-statis` **227 menjadi 224**, dan ketiganya disengaja: dua dari lahan (6 baris menjadi 4) dan satu dari `/master/referensi/jenis_dokumen_lahan` yang enumnya dicabut. Bukan tautan mati - diperiksa satu per satu, sebab angka yang turun tanpa penjelasan adalah cara termudah menyembunyikan regresi.
- [done 2026-09-02] **UI multi-unggah berkas, dan dua kontrol mati yang baru ketahuan karenanya (Putaran 14).**
  * **Putaran 12 hanya membuat multi-berkas MUNGKIN, belum TERPAKAI.** Registrinya sanggup memegang banyak berkas per pemilik sejak 2026-09-02 pagi, tetapi setiap form masih memasang isian tunggal, sehingga kemampuan itu tidak dapat dicapai petugas dari layar mana pun. Keputusan pemilik proyek: kerjakan komponennya dulu dengan tiga domain percontohan, bukan dua belas domain sekaligus.
  * **Komponen baru `x-sim.berkas-unggah`, dan `x-sim.file-upload` TIDAK dicabut.** Dua puluh dua titik unggah lain memang bersifat satu berkas (satu SK penetapan SP, satu foto profil), dan memaksanya jamak akan menyulitkan tanpa memberi apa pun. Yang jamak hanya yang memang jamak di lapangan.
  * **Tiga domain percontohan dipilih sebab bentuk jamaknya berbeda-beda:** infrastruktur (satu irigasi 1,2 km punya beberapa TITIK kerusakan), pengaduan (beberapa SUDUT pemotretan atas satu kejadian), transmigran (KTP, KK, dan SK adalah dokumen BERBEDA JENIS, bukan banyak salinan hal yang sama). Ketiganya menguji sisi komponen yang berlainan.
  * **Batas 5 MB diperiksa PER BERKAS dan pesannya menyebut nama berkasnya.** Petugas yang memilih lima foto sekaligus lalu menerima "ukuran melampaui batas" tanpa nama tidak tahu mana yang harus diganti, dan biasanya mengulang kelimanya.
  * **Dua kontrol mati (R-26) baru ketahuan justru karena pekerjaan ini.**
    - Panel Dokumen pada rincian transmigran masih membaca kolom `dokumen_pendukung` yang sudah dicabut Putaran 12, sehingga menampilkan "Belum ada dokumen" padahal empat berkas (SHM, KTP, KK, SK) benar-benar ada di registry. Unggahan tanpa jalan dibuka, persis bentuk yang dilarang R-26. Sekarang membaca registry.
    - Rincian infrastruktur menautkan `$data['foto']` yang hanya memuat satu nama, sehingga dua foto titik kerusakan lain tidak dapat dibuka. Sekarang `@forelse` atas seluruh berkas berperan `foto`.
  * **Tautan kembar dicegah dengan sengaja.** Halaman rincian meng-include formnya sendiri, sehingga komponen yang memasang tautan buka menerbitkan dua tautan ke berkas yang sama (uji sempat menghitung 5, bukan 2). Keputusan: tautan adalah milik PANEL RINCIAN, form cukup menampilkan nama berkas tersimpan. Prop `modul`/`idPemilik` yang sempat dibuat dicabut lagi.
  * **`wajib` berarti PALING SEDIKIT satu, bukan selalu.** Isian KK ditandai `required` hanya ketika belum ada berkas tersimpan; bila sudah ada, menyunting nomor telepon tidak boleh memaksa pengunggahan ulang KK.
  * **Tiga penjaga uji diperbarui atas persetujuan pemilik proyek, bukan disesuaikan diam-diam.** Ketiganya memeriksa hal yang masih berlaku, tetapi mengasumsikan satu berkas: pola `name="foto"` kini menerima `name="foto[]"`; jumlah tautan rincian infrastruktur 2 menjadi 4; dan `dokumen_pendukung` dicabut dari daftar kolom form transmigran, diganti penjaga baru yang menuntut `ktp[]`, `kk[]`, `sk[]` ada DAN `dokumen_pendukung` tidak ada lagi.
  * **Satu suplai data dibuang sebagai kode mati.** `berkasBukti` sempat ditaruh pada rute `pengaduan.detail`, padahal rincian pengaduan tidak pernah meng-include formnya; form pengaduan hanya hidup di halaman indeks. Dicabut daripada dibiarkan menyesatkan pembaca berikutnya.
  * **Privasi lacak publik diperiksa, tidak diasumsikan.** `pages.publik.lacak` adalah view terpisah dari form internal dan tidak menerima berkas apa pun, sehingga bukti pengaduan tetap tidak bocor ke halaman tanpa login.
  * **Uji: 727 PASS, 7.415 assertions.** `sim:tautan-statis` tetap 227, `pint` tetap 33 berkas pre-existing (dua berkas sempat naik karena CRLF dari PowerShell, sudah dinormalkan ke LF).
  * **Lanjutan: rencana "9 domain sisanya" TERNYATA SALAH, dan pemeriksaan skema yang membetulkannya.** Catatan penutup di atas menyebut sembilan domain berpivot masih menunggu. Setelah `schema.sql` dibaca sungguhan, angka itu keliru pada dua sisi sekaligus:
    - **Yang disebut pivot padahal BUKAN:** poktan, saprotan, dan SP memakai **FK langsung** (14a.8c) sebab berkasnya memang selalu satu. Menjamakkannya berarti melawan skema yang baru ditetapkan, bukan melengkapinya.
    - **Yang pivot tetapi tetap tunggal:** `user_berkas` ber-`UNIQUE (user_id)` saja, jadi satu foto profil per pengguna adalah batas yang disengaja (14a.8d).
    - **Yang tidak dapat dijamakkan sekarang:** `penanganan_pengaduan` tidak punya kolom id sama sekali pada `DummyData` (barisnya dikunci nomor pengaduan), persis pengecualian yang sudah dicatat Putaran 12. Pivotnya tidak punya apa pun untuk dicocokkan.
    - **Yang benar-benar layak jamak:** hanya **empat** - kawasan, rumah, inventaris SP, dan fasilitas SP. Keempatnya dikerjakan.
  * **Kontrol mati KETIGA ditemukan: kawasan.** Kawasan tidak punya halaman rincian sendiri, sehingga kartu pada `/kawasan` adalah satu-satunya tempat berkasnya dapat dibuka - dan kartu itu hanya menampilkan SK. HPL dan peta cakupan sudah ada di registry sejak Putaran 12 tetapi **tidak dapat dicapai dari mana pun**. Sekarang ketiganya tertaut.
  * **Kontrol mati KEEMPAT: panel dokumentasi rumah.** Penjaga kosongnya memeriksa `$data['foto_rumah']` yang sudah tidak terisi, sehingga foto dari registry tidak pernah dirender meski datanya ada. Bentuknya sama persis dengan panel Dokumen transmigran; ketahuan hanya karena uji baru menuntut dua tautan dan mendapat nol.
  * **`rumah_berkas` belum pernah punya data contoh sama sekali.** Pivotnya ada di `schema.sql` sejak Putaran 12, tetapi tanpa satu baris pun data, bentuk jamaknya tidak pernah teruji di layar. Ditambah dua foto (tampak depan, dinding retak).
  * **Dua berkas Blade sempat rusak oleh penyuntingan berbasis nomor baris.** Penghapusan rentang meleset satu baris dan mencabut `<div>` pembuka pada form fasilitas dan form kawasan. Penjaga `menyeimbangkan tag HTML` menangkapnya seketika - justru contoh mengapa penjaga semacam itu ditulis.
  * **Tiga penjaga BARU ditambahkan**, bukan sekadar menyesuaikan angka: `/transmigran/2` (4 berkas), `/rumah/1` (2 foto), dan `/kawasan` (3 berkas). Ketiganya menutup kontrol mati yang baru diperbaiki agar tidak diam-diam kembali.
  * **Uji akhir: 730 PASS, 7.421 assertions.** `sim:tautan-statis` 227, `pint` 33 pre-existing, dan seluruh sebelas halaman berberkas diperiksa **nol tautan kembar**.
- [done 2026-09-02] **Registry berkas terpusat, dan pembetulan penempatan HPL/SHM yang keliru sejak awal (Putaran 12).**
  * **Pemicu:** audit khusus upload berkas menyimpulkan desain lama `MEMADAI DENGAN PERBAIKAN KECIL` dan `dokumen_lahan` adalah pengecualian yang tepat. Diskusi dengan pemilik proyek membalik kesimpulan itu, dan pembaliknya benar.
  * **Kesalahpahaman yang ditemukan.** `rules.md` 7.4a sebenarnya SUDAH menulis bahwa HPL adalah Hak Pengelolaan milik instansi atas tanah kawasan dan `tidak pernah menjadi hak seorang transmigran`, tetapi tabel `dokumen_lahan` menempelkannya ke tiap bidang. Dari situlah lahir pivot m2m `dokumen_lahan_bidang`: ia menambal AKIBAT, bukan sebab. SHM pun meliputi seluruh lahan satu KK, pekarangan maupun usaha, sehingga menempelkannya ke `lahan_id` memaksa satu sertifikat diunggah dua kali per keluarga.
  * **Setelah keduanya ditempatkan benar, kedua tabel hilang TANPA kehilangan kemampuan apa pun.** HPL ke `kawasan_transmigrasi`, SHM ke `transmigran`. Bukan dikorbankan demi keseragaman, melainkan memang salah tempat. Ini pembalikan Putaran 7 poin G.
  * **`transmigran.status_sertifikat`** (`Sudah`/`Belum`/`Belum Didata`) ditambahkan atas pertanyaan pemilik proyek yang tepat: ketiadaan unggahan TIDAK boleh disimpulkan sebagai belum bersertifikat, sebab ia dapat berarti sudah punya tetapi belum diunggah petugas. Nilai ketiga memisahkan keduanya.
  * **Registry `berkas` menggantikan 24 kolom path** yang tersebar di 17 tabel. Alasan pokoknya bukan keseragaman: kolom path telanjang TIDAK merekam `mime` maupun `ukuran`, padahal 14a.1 dan 14a.2 mewajibkan keduanya divalidasi di sisi server. Tanpa merekamnya, tidak ada cara memeriksa ulang apa yang sebenarnya tersimpan.
  * **BUKAN tabel polymorphic, dan itu keputusan yang menentukan.** Tanpa `entity_type`/`entity_id`: kepemilikan dinyatakan 12 pivot per domain dan 5 FK langsung. Polymorphic akan mencabut dua hal yang baru ditetapkan sebagai fondasi: integritas referensial yang ditegakkan MySQL, dan penyaring cakupan data tunggal (5.0b-1 poin 8) yang menempel pada model pemilik SP.
  * **Penamaan `berkas`, bukan `dokumen`**, atas usulan pemilik proyek sebab tabelnya menampung foto juga. Berbasis data, bukan selera: `berkas` muncul 302 kali di kode, `file` hanya 45, dan 4.0 mewajibkan nama tabel Bahasa Indonesia. Diangkat tepat waktu, sebelum 25 titik frontend memakainya.
  * **Siklus FK ditemukan dan diputus.** `berkas.user_id` menunjuk `user`, sedangkan rencana awal `user.foto_berkas_id` menunjuk balik. Tidak ada urutan `CREATE TABLE` yang memenuhi keduanya, padahal kepala `schema.sql` menuntut parent sebelum child. Diputus dengan memakai pivot `user_berkas` ber-UNIQUE pada `user_id` saja.
  * **`is_gcs` diganti `disk VARCHAR`.** Boolean hanya sanggup dua keadaan, sedangkan menyimpan nama disk Laravel membuat `Storage::disk()` langsung terpakai dan penyimpanan ketiga tidak menuntut kolom baru. `public_link` dan `updater` dibuang: yang pertama melawan 14a.6, yang kedua kalah dari `audit_log` yang sudah menyimpan `data_lama` dan `data_baru`.
  * **Lahan: TEPAT 1 pekarangan + 1 usaha, kini ditegakkan basis data** lewat `UNIQUE (transmigran_id, peruntukan_lahan)`. Membalik 7.9 yang dahulu menyatakan jumlahnya `kewajaran, bukan batas`. Konsekuensi yang diterima sadar: lahan usaha di dua petak terpisah tidak lagi dapat didata.
  * **`pola_tanam`, `peralatan_pertanian`, dan `kendala` dicabut** beserta tab Pengelolaan yang hanya menampung ketiganya. Kolom `Pola Tanam` pada Laporan Transmigran diganti `Status Sertifikat` yang lebih berguna bagi dinas. Rekap Monografi yang dahulu memakai `pola_tanam` sebagai penanda lahan diusahakan kini membacanya dari peruntukannya sendiri.
  * **Skema diverifikasi dengan MENJALANKANNYA, bukan membacanya.** `schema.sql` diimpor ke MariaDB 10.4 sebagai basis data sementara: 61 tabel terbentuk, 94 foreign key benar-benar dibuat engine (17 di antaranya menunjuk `berkas`), dua bidang beda peruntukan diterima, dan bidang ketiga berperuntukan sama DITOLAK dengan `Duplicate entry '1-Lahan Usaha'`. Basis data uji dibersihkan sesudahnya.
  * **View tidak disunting satu per satu.** `DummyData::lekatkanBerkas()` menempelkan nama berkas dari registry ke kunci lamanya (`foto`, `dokumen_pendukung`), sehingga 25 titik frontend tetap bekerja tanpa perubahan; yang berpindah hanya ASAL nilainya. Tiap kunci dapat pendamping `*_meta` berisi metadata penuh, siap dipakai saat UI multi-unggah dibangun.
  * **Satu pengecualian yang perlu dicatat:** `penangananPengaduan()` tidak memiliki kolom id sama sekali (barisnya dikunci nomor pengaduan), sehingga pivot tidak dapat mencocokkannya. Ketiga dokumen tindak lanjutnya dibaca langsung lewat `cariBerkas()`.
  * **Uji: 727 PASS.** Perjalanannya 12 gagal, lalu 4, lalu 0. Lima penjaga usang diganti penjaga baru yang menegakkan penempatan yang benar; dua di antaranya kini justru memeriksa bahwa `dokumenLahan` TIDAK lagi ada. Satu uji menuntut fasilitas 3 punya dua tautan berkas, dan itu benar - datanya yang kurang, sehingga foto kondisinya ditambahkan.
  * **Verifikasi lain:** `sim:tautan-statis` tetap 227, `pint` kembali ke 33 berkas pre-existing setelah akhir baris dirapikan, seluruh berkas tersunting bebas BOM.
- [done 2026-09-02] **Nomor pengaduan memuat bagian acak, dan konflik format pada CMS diselesaikan.**
  * **Masalah:** `rules.md` 4.0a poin 4 sudah mewajibkan bagian acak sejak lama, tetapi data contoh masih memakai `PGD-2026-0001` yang berurutan. Halaman lacak dapat dibuka TANPA LOGIN, sehingga nomor berurutan berarti siapa pun dapat menyusuri `PGD-2026-0001` sampai `PGD-2026-9999` dan memanen status seluruh laporan warga.
  * **Uji privasi yang sudah ada tidak menutup celah ini.** Ia menjaga ISI halaman lacak (nama pelapor, kontak, bahkan nama petugas tidak dirender), tetapi tidak menjaga SIAPA yang dapat sampai ke sana. Keduanya diperlukan dan tidak saling menggantikan.
  * **Bentuknya `PGD-2026-0001-PMTUXK`:** enam karakter acak beralfabet tanpa `0`, `O`, `1`, `I`, dan `L`. Warga membaca nomornya dari layar ponsel lalu menyalinnya ke halaman lacak, dan salah baca satu karakter membuat laporannya seolah tidak pernah ada. Nomor urutnya DIPERTAHANKAN sebab masih berguna bagi petugas untuk mengurutkan dan menyebut laporan; yang menutup penyusuran adalah bagian acaknya.
  * **Konflik CMS.** `pages/cms/index.blade.php` menyediakan isian format tiket bernilai `PGD-{TAHUN}-{NOMOR}` tanpa bagian acak, artinya CMS menjanjikan dinas dapat menetapkan format yang justru melanggar 4.0a poin 4. Keputusan pemilik proyek: bagian acak WAJIB dan di luar kendali CMS. Dinas mengatur awalan dan pola nomor urut; suffix acak selalu ditambahkan sistem. Templatenya diperbarui beserta keterangan tampil yang menyatakan bagian `{ACAK}` tidak dapat dihilangkan, lengkap dengan alasannya.
  * **`rules.md` 4.0a mendapat poin 4a sampai 4d:** bagian acak di luar kendali CMS, bentuk enam karakter beralfabet aman, nomor urut dipertahankan, dan nomor tidak pernah berubah setelah terbit sebab warga sudah mencatat atau memotretnya.
  * **Perkiraan 21 titik uji ternyata hanya 5 yang gagal.** Sisanya lolos lewat pencocokan sebagian: `assertSee('PGD-2026-0005')` tetap cocok terhadap `PGD-2026-0005-96RY4X`. Kelimanya dibetulkan, dan **sembilan titik yang sebenarnya lolos ikut dibereskan** sebab titik yang lolos hanya karena kebetulan awalannya sama akan pecah diam-diam pada perubahan data berikutnya.
  * **Ujinya menjadi lebih kuat, bukan dilonggarkan.** Nomor kini dibaca dari `DummyData` lewat penyaring yang sama dengan yang sedang diuji, sehingga uji penyaring status dan penyaring bidang menegakkan HUBUNGAN status dengan barisnya, bukan sekadar mencocokkan teks harfiah. Yang tersisa di berkas uji hanya satu komentar riwayat, dan itu memang catatan bukan nilai yang diuji.
  * **Dua temuan di luar rencana.** Placeholder `publik/lacak.blade.php` masih menampilkan `Contoh: PGD-2026-0001` tanpa suffix; bila dibiarkan, contoh yang ditunjukkan kepada warga justru mengajarkan bentuk yang salah sehingga nomor yang diketik menurutnya tidak akan pernah ditemukan. Lalu uji penyaring bidang sempat saya tulis membaca `bidang_penanganan` padahal kolomnya bernama `bidang`; tertangkap uji, bukan lolos.
  * **Nama berkas dokumen tindak lanjut TIDAK diubah** (`BeritaAcaraPeninjauan_pgd-2026-0001.pdf`). Ia path berkas yang sudah tersimpan di disk, bukan nomor yang ditampilkan atau dicari; mengubahnya berarti menunjuk berkas yang tidak ada.
  * **Verifikasi:** 729 uji (7.410 assertions) hijau, `sim:tautan-statis` tetap 227 sebab tautannya terbangkit dari data sehingga ikut menyesuaikan sendiri, `pint` 33 pre-existing. Halaman lacak bernomor penuh, `/pengaduan`, `/pengaduan-warga`, dan `/cms` seluruhnya 200.
- [done 2026-09-02] **Rancangan penegakan cakupan data, keputusan UUID, dan pembetulan Task 3.2.** Seluruhnya dokumen; tidak ada kode aplikasi yang disentuh.
  * **Kekosongan yang ditutup.** `rules.md` 5.0b poin 6 sudah mewajibkan cakupan ditegakkan sebagai penyaring query, tetapi tidak pernah menyatakan DI MANA penyaringnya dipasang. Tanpa penetapan itu, penegakannya akan tersebar ke tiap tempat pengambilan data satu per satu, dan satu tempat yang terlewat tidak memerahkan apa pun: datanya tampil, tidak ada galat, tidak ada yang tahu.
  * **Angka yang menentukan rancangan.** Laporan audit sempat menyatakan pengambilan data terpusat di `ViewServiceProvider`. Pencacahan ulang membantahnya: `routes/web.php` memuat **167** pemanggilan `DummyData` yang mencakup 65 metode berbeda, sedangkan `ViewServiceProvider` hanya 59. Yang benar hanyalah bahwa view tidak mengambil datanya sendiri. Dengan 167 titik, penegakan per pemanggilan berarti 167 peluang terlewat, dan itulah alasan titik penegakannya dipilih berupa **global scope pada model**, bukan penyaringan di pemanggil.
  * **Ditetapkan pada `rules.md` 5.0b-1 (sembilan poin, mengikat Tahap 3):** penyaring dipasang pada pemilik SP bukan diulang pada tiap turunannya; pengadaan `alsintan`/`saprotan` induk tidak disaring sebab belum berada di SP mana pun sehingga menyaringnya menyembunyikan barang gudang UPT dari semua orang; data referensi tidak pernah disaring; akun `Per SP` tanpa penugasan menerima NOL baris; data tak berhak membalas 404 bukan 403; penyaringan mendahului paginasi; dan penegakannya wajib disertai uji penjaga.
  * **Tiga jebakan yang ditulis eksplisit sebab paling mudah terjadi.** Pertama, daftar SP kosong secara naluriah diterjemahkan menjadi tanpa syarat, sehingga akun yang paling tidak berhak justru melihat segalanya. Kedua, 403 menyatakan barisnya ADA tetapi tidak boleh dibuka, dan pernyataan itu sendiri kebocoran. Ketiga, menyaring koleksi hasil setelah query membuat jumlah halaman membocorkan banyaknya data SP lain meski isinya tidak tampil.
  * **Angka rekap yang menyempit wajib menyatakan cakupannya.** Operator SP melihat angka SP-nya sendiri, bukan angka kawasan; tanpa keterangan itu angkanya dapat disalin ke laporan sebagai total kawasan. Alasannya sama dengan kewajiban menulis periode dan cakupan pada rekap panen (9 poin 8b dan 8o).
  * **UUID: dikerjakan bersamaan dengan pembuatan Model, bukan sesudahnya** (`rules.md` 4.0a poin 5a sampai 5c). Pada saat Model dibuat, penggantiannya cukup satu `getRouteKeyName()` per model; setelah controller dan rute terlanjur ditulis di atas id integer, penggantiannya menuntut penyisiran tiap `route()`. Rute tahap frontend sengaja TIDAK diubah sekarang: belum ada Model yang menerjemahkan pengenal publik menjadi kunci internal, sehingga penerjemahannya terpaksa ditulis tangan lalu dibuang lagi. Pembatas rute `[0-9]+` wajib ikut disesuaikan, sebab pembatas angka menolak UUID dan menghasilkan 404 yang membingungkan.
  * **Task 3.2 dibetulkan.** Butirnya masih berbunyi `email atau NIK` padahal catatan di atasnya sudah menandainya usang sejak 2026-09-01. Pembaca yang melompat langsung ke butir itu tetap membaca ketentuan yang keliru, dan itulah sebabnya penandaan usang di tempat lain tidak cukup.
  * **Verifikasi:** 729 uji tetap hijau tanpa satu pun penyesuaian, `sim:tautan-statis` 227, `pint` 33 pre-existing. Diff atas `app`, `resources`, `routes`, dan `tests` tidak bertambah satu baris pun, sebagaimana seharusnya bagi bagian yang hanya menyunting dokumen.
- [done 2026-09-02] **Master Wilayah menjadi satu datatable, dan form SP menyaring desa lewat kawasan.**
  * **Empat tab dicabut.** Halaman `/wilayah` memakai tab per tingkat dengan `x-sim.tabel-ringkas` yang merender SELURUH baris tanpa pencarian maupun paginasi. Begitu provinsi dan kabupaten dibaca dari data referensi nasional, tab Kabupaten memuat 514 baris dan tidak dapat dipakai. Mencari satu nama juga menuntut petugas menebak lebih dulu ia berada di tab mana, padahal yang ia tahu hanya namanya.
  * **Tab sudah dua kali melahirkan cacat** dan keduanya tercatat di kode: tab bawaan yang keliru (`wilayah.blade.php`), dan tingkat form yang tidak mengikuti tab yang sedang terbuka (`form-wilayah.blade.php`). Menyatukannya menghapus kelas cacat itu, bukan memperbaikinya untuk ketiga kali.
  * **Filter Tingkat WAJIB mencantumkan jumlah.** Judul tab lama menampilkan angka itu; menghapus tab tanpa memindahkan angkanya berarti pembaca kehilangan keterangan yang sebelumnya ada, dan perombakan berubah menjadi kemunduran. Pencarian mencakup nama, induk, dan kode, sebab petugas kerap mengingat kabupatennya ketika nama kecamatannya sendiri sudah kabur.
  * **`kawasan` mendapat `kabupaten_id`.** Sebelumnya hanya teks `'kabupaten' => 'Malaka'`, yaitu cacat yang sama dengan blocker B-1 dan `daerah_asal`. Skema sudah lebih dulu benar (`fk_kawasan_kabupaten`).
  * **Form SP: Kawasan sebelum Desa,** sepola dengan form Rumah dan form Lahan yang menaruh penentu sebelum yang ditentukan (`rules.md` 6a.12 dan 7.12). Memilih kawasan menyaring desa menjadi desa pada kabupaten kawasan itu.
  * **Penyaringannya menempuh KABUPATEN, bukan relasi kawasan ke desa.** Pemilik proyek sempat meminta desa disaring langsung menurut kawasan, lalu mengoreksi sendiri: kawasan dan desa adalah dua cabang terpisah yang baru bertemu di SP (`rules.md` 4a.2). Menautkannya langsung berarti mengarang relasi yang memang sengaja tidak dimodelkan. Rantai yang dipakai sudah ada seluruhnya: kawasan -> kabupaten, dan desa -> kecamatan -> kabupaten.
  * **`desaBerkabupaten()`** menurunkan kabupaten desa lewat kecamatannya, TIDAK menyimpannya. Menyimpannya berarti dua kolom yang saling menentukan dapat berselisih diam-diam ketika satu kecamatan dipindah kabupaten.
  * **Tiga uji penjaga diarahkan ulang, tidak satu pun dilonggarkan.** Dua uji tab diganti penjaga baru untuk datatable, sehingga yang dijaga berpindah mengikuti fiturnya: kini justru menegakkan bahwa tab TIDAK ADA lagi, jumlah per tingkat tetap terbaca, dan penyaring benar-benar menyempitkan. Uji ketiga dipertahankan tetapi pemeriksaannya dipindah dari teks sumber ke HTML hasil render, sebab `x-sim.pilih-cari` merender `name=` di dalam komponennya sehingga pemeriksaan berbasis sumber mengunci CARA implementasi, bukan perilakunya.
  * **Assertions naik 6.377 menjadi 7.410** dengan jumlah uji tetap 729. Penjaga penggantinya menguji lebih banyak daripada yang digantikan.
  * **Dua kekeliruan saya, keduanya tertangkap sebelum uji dijalankan.** Ekspresi Alpine sempat ditulis berawalan titik dua sehingga Blade menilainya sebagai PHP (`Undefined constant tingkat`, halaman 500); komponen membacanya lewat `\->get()` sehingga harus berupa atribut biasa. Lalu `petaKawasanKabupaten` dipakai form SP sebelum didaftarkan pada daftar kunci `pages.sp.form`, dan `/sp` berbalas 500 - sejenis dengan tabrakan nama pada D1.
  * **Verifikasi:** 729 uji hijau, `sim:tautan-statis` tetap 227, `pint` kembali ke 33 berkas pre-existing setelah akhir baris berkas tersunting dirapikan. Halaman `/wilayah`, `/sp`, `/sp/1`, `/kawasan`, dan `/transmigran` seluruhnya 200; kabupaten, kecamatan, dan desa nihil yatim.
  * **Kecamatan dan desa penuh TETAP DITUNDA ke Tahap 3.** Alasannya menguat setelah bagian ini: `pilih-cari` menyematkan seluruh opsi ke dalam HTML, wajar untuk 514 kabupaten tetapi tidak untuk 83.000 desa. Pemuatan bertahap lewat endpoint baru tersedia setelah backend berdiri.
- [done 2026-09-02] **`daerah_asal` menjadi FK `kabupaten`, bukan teks bebas.**
  * **Masalah:** `transmigran.daerah_asal` berupa `VARCHAR(255)` teks bebas TANPA indeks, padahal ia salah satu dari enam dasar rekap kependudukan (`rules.md` 10a.4a). Pada data nyata ejaannya beragam, dan `KUPANG`, `Kab. Kupang`, serta `KABUPATEN KUPANG` menjadi tiga baris rekap berbeda meski menunjuk tempat yang sama. `UppercaseInput` menyeragamkan huruf besarnya, tetapi tidak ejaannya. Kegagalannya SENYAP: totalnya tetap benar, sehingga tidak ada yang menyadari pembagiannya bocor. Cacat ini sudah diakui pada komentar `sebaranDaerahAsal()` sejak lama tetapi belum pernah ditangani.
  * **Nama kabupaten tidak unik secara nasional,** dan itu alasan kedua yang lebih menentukan. Kabupaten Kupang (5301) berbeda dari Kota Kupang (5371), sedangkan data contoh hanya menulis `KUPANG` sehingga tidak dapat dibedakan. Karena itu isiannya memakai searchable dropdown yang menampilkan nama provinsi pada baris kedua.
  * **Memakai tabel `kabupaten`, BUKAN `referensi`.** Tabel `provinsi` dan `kabupaten` sudah ada pada skema lengkap dengan UNIQUE komposit `(provinsi_id, nama)`, yaitu justru penjaga yang dibutuhkan untuk nama kembar. Menaruhnya di `referensi` berarti membuat daftar wilayah KEDUA yang terpisah dari hierarki wilayah yang sudah ada; `referensi` adalah daftar datar untuk nilai enum-like, sedangkan wilayah administratif punya hierarki.
  * **`pekerjaan_kepala_keluarga` SENGAJA tidak ikut diubah.** Audit sempat menggabungkan keduanya sebagai satu temuan, dan itu keliru. Pekerjaan adalah himpunan TERBUKA dengan ekor panjang yang sah, sehingga mengunci ke data master akan menghalangi petugas ketika menemui pekerjaan yang belum terdaftar; keputusan `schema.sql` (teks bebas + datalist) sudah benar. Daerah asal himpunan TERTUTUP. Memperlakukan keduanya sama adalah kekeliruan analisis yang dikoreksi di sini.
  * **`DataWilayah.php` baru** memuat 38 provinsi + **514** kabupaten/kota dari `database/data/wilayah_indonesia.sql`. Berdiri terpisah dari `DummyData` sebab ia data REFERENSI yang tetap dipakai setelah backend berdiri, bukan simulasi transaksi. Nama disimpan apa adanya beserta awalan Kabupaten/Kota; awalan itu bukan hiasan melainkan pembeda Kabupaten Kupang dari Kota Kupang.
  * **Berkas sumber memuat 38 baris PROVINSI yang nyasar ke tabel `t_kota`** (berkode 2 digit). Setelah disaring menurut panjang kode, hasilnya 514, dan itu jumlah resmi Indonesia. Rencana semula menduga hanya 1 baris nyasar; dugaan itu salah dan ketahuan saat pemeriksaan data, bukan saat perencanaan.
  * **Kecamatan dan desa penuh DITUNDA ke Tahap 3** atas keputusan pemilik proyek. Berkas sumber memuat 7.000 kecamatan dan 83.000 kelurahan, sedangkan komponen `pilih-cari` menyematkan seluruh opsi ke dalam HTML. Kecamatan dan desa LOKUS tetap pada `DummyData::wilayah()`, sebab hanya wilayah ber-SP yang bermakna di sana.
  * **`sebaranDaerahAsal()` kini berkunci `kabupaten_id`,** dengan `sebaranDaerahAsalBerlabel()` sebagai satu-satunya tempat pelabelan. Tanpa pemisahan itu tiap pemakainya melabeli sendiri-sendiri, dan pelabelan yang keliru gagal senyap: id yang tidak ketemu tampil sebagai tanda hubung, dan tanda itu terbaca `tidak ada data` padahal artinya `kodenya tidak menemukan namanya` (pola yang sama dengan `rules.md` 9 poin 8i). Kunci `Lainnya` tetap berupa teks sebab ia penampung sisa, bukan satu kabupaten tertentu.
  * **FK memakai `ON DELETE RESTRICT`:** merapikan data master tidak boleh melenyapkan daerah asal seorang transmigran. Kolomnya NULL-able sebab data lama boleh belum terisi.
  * **Dua kekeliruan saya tertangkap uji, dan keduanya layak dicatat.** Pertama, suplai view baru saya namai `daftarKabupaten` padahal nama itu sudah dipakai `pages.sp.form-kawasan` dengan bentuk berbeda; deklarasi saya menimpanya dan `/kawasan` berbalas HTTP 500 (diperiksa lewat `git stash`: 200 sebelum, 500 sesudah). Diganti `opsiDaerahAsal`. Kedua, `HalamanTest` rekap kependudukan mencari `5321` pada halaman yang merender `Kabupaten Malaka`; ujinya BENAR dan tidak dilonggarkan, yang dibetulkan adalah sumbernya menjadi varian berlabel.
  * **Temuan sampingan:** `pages/laporan/isi/transmigran.blade.php` masih menyusun `petaRumah` berkunci NAMA penghuni, yaitu cacat yang sama dengan blocker B-1 tetapi di tempat berbeda dan terlewat pada audit awal. Ikut dibetulkan menjadi `transmigran_id`.
  * **Verifikasi:** 729 uji (6.377 assertions) hijau, `sim:tautan-statis` tetap 227, `pint` kembali ke 33 berkas pre-existing setelah berkas baru dirapikan. `data-dictionary.md` dan uji penjaga nama kolom diperbarui BERSAMA, sebab penjaga itu memang bertugas menjaga keduanya sinkron.
  * **Kewajiban tahap backend:** data lama bertuliskan teks bebas TIDAK dapat dipetakan otomatis ke id tanpa menebak. Impor data produksi menuntut langkah pemetaan manual, bukan konversi diam-diam.
- [done 2026-09-02] **Audit menyeluruh pra-backend: rujukan skema, penautan rumah, unggahan berkas, rute ganda.**
  * **Pemicu:** pemilik proyek meminta audit menyeluruh sebelum tahap backend dimulai. Rincian rencana beserta penyisiran lima sudut ada pada `session-notes.md` Putaran 11.
  * **`rumah` menaut penghuni lewat NAMA, bukan id (blocker).** `DummyData::rumah()` tidak memiliki `transmigran_id` sama sekali; penautan dilakukan dengan mencocokkan `nama_kepala_keluarga`. Skema sudah benar lebih dulu (`uq_rumah_transmigran` pada `database/data/schema.sql`), hanya data contoh dan frontend yang tertinggal.
    - **Mengapa ini bukan sekadar kerapian:** suksesi kepala keluarga MENYUNTING baris `transmigran` yang sama (`rules.md` 6.5), sehingga namanya berubah. Sejak saat itu `rumah.penghuni` tidak lagi cocok dengan siapa pun: rumahnya tetap tampil, penghuninya hilang, tanpa satu pun pesan galat. Dua KK bernama sama juga cukup untuk menautkan rumah ke keluarga yang keliru.
    - **Bukti bahwa cacatnya sudah aktif:** `transmigranTanpaRumah()` mengembalikan 8 dari 8 KK, sebab pencocokan namanya tidak pernah berhasil. Artinya dropdown pemilihan penghuni pada form rumah tidak pernah sekali pun menawarkan daftar yang benar. Sesudah diperbaiki: `[5,6,7,8]`, dan `rumahKosong()` = `[3,6]`.
    - Form rumah ternyata sudah setengah benar: `name=transmigran_id` dan `kunci=id_transmigran` sudah dipakai, hanya penyusun `\$calonPenghuni` yang masih memakai nama. Akibatnya pada modal ubah, penghuni yang sedang menempati rumah tidak pernah tersisip ke daftar pilihannya sendiri.
    - Rumah 3 diisi `transmigran_id` null beserta komentar: ia pernah dihuni DOMINGGUS TAEK sampai 2025-09-30, dan jejaknya tetap ada pada `riwayatPenghunian()`. Kolom ini menyimpan penghuni SEKARANG, bukan riwayat.
  * **Ekstensi berkas unggahan ditentukan klien.** `PenyimpananDokumen::susunNamaBerkas()` memakai `getClientOriginalExtension()`, yang hanya memenggal nama kiriman peramban, sehingga berkas apa pun dapat diberi nama `.pdf` maupun `.php`. Belum dapat dieksekusi sebab disknya privat dan `DokumenController` mengalirkan isinya, tetapi ia menjadi celah nyata begitu ada satu saja disk publik atau symlink ke folder itu.
    - Diganti `ekstensiAman()`: menebak dari isi berkas lewat `extension()` lebih dulu, nama klien hanya cadangan, dan hasilnya SELALU disaring daftar putih. Daftar putihnya dibaca dari `ValidationRules::JENIS_BERKAS` yang sudah ada, bukan daftar baru, agar tidak lahir dua sumber kebenaran yang dapat berselisih diam-diam (`rules.md` 14a poin 2 dan 13.2 poin 6).
    - Diuji dengan berkas nyata: PNG asli bernama `.php` kini tersimpan sebagai `.png`, dan PHP murni bernama `.php` ditolak.
  * **Nama rute `sp.perbarui` terdaftar dua kali.** Keduanya `PUT /sp/{id}`; Laravel memakai yang terakhir, sehingga yang berlaku adalah versi ber-`redirect()->route('sp.index')`. Akibatnya menyunting SP dari halaman rincian melempar petugas ke daftar SP beserta hilangnya posisi tab, padahal `rules.md` 4c poin 5 justru menjamin posisi tab bertahan saat modal tersimpan. Deklarasi kedua dicabut, versi ber-`back()` dipertahankan sebab ia tetap benar dari halaman daftar maupun dari halaman rincian.
  * **Rujukan berkas skema rusak di 4 dokumen (8 titik).** `database/transmigrasi.sql` tidak pernah ada di repositori; berkasnya bernama `database/data/schema.sql`. Titik terpenting ada pada `tasklist.md` yang menyebutnya acuan seluruh migration Tahap 3 sampai 9, yaitu tepat pekerjaan berikutnya. Seluruhnya dibetulkan; nama lama dipertahankan di `notes.md` sebagai riwayat pemindahan.
  * **Angka 44 tabel bisnis pada dokumen sudah BENAR** (50 `CREATE TABLE` dikurangi 6 tabel infrastruktur Laravel). Temuan audit yang sempat menyebut 51 tabel keliru, sebab pencacahannya ikut menangkap baris komentar.
  * **Tujuh duplikasi kunci array** `satuan_permukiman_id` pada `DummyData::transmigran()` dihapus. Tidak berdampak pada perilaku sebab nilainya kembar, tetapi 729 uji tidak menangkapnya: uji memeriksa NILAI, bukan BENTUK data. Menjadi alasan mengapa penyisiran manual tetap diperlukan.
  * **Verifikasi:** 729 uji Pest (6.377 assertions) hijau, sama dengan sebelum putaran ini. Rute 152 menjadi 151, dan pemeriksaan menyeluruh memastikan tidak ada nama rute ganda lain. `pint --test` tetap 33 berkas gagal seperti sebelumnya (pre-existing, di luar cakupan). `sim:tautan-statis` tetap 227 tautan.
...
- [done] Pada halaman alsintan, tambahkan field form “Penerima” dari Poktan (bisa dari ketua/anggota).
  * **Selesai 2026-08-27.** Kolom `penanda_terima_id` menunjuk `anggota_poktan.id`, pilihannya ikut poktan terpilih dan hanya anggota aktif — ketua maupun anggota biasa. Ini **penanda tangan serah terima, BUKAN pemilik**: alat tetap milik kelompok, `rules.md` 7b.1 tidak disentuh (dikonfirmasi pemilik proyek). Namanya dihitung dari `anggotaPoktan()`, tidak disimpan. Lihat bagian 1m.5.
- [done] Lalu sepertinya untuk halaman Satuan Pemukiman bakal ada tambahan kolom/field form baru. Untuk field form-nya bisa coba kita diskusikan dulu berdasarkan Bab II Keadaan Wilayah pada Laporan Monografi di folder refs. Soalnya nanti datanya ini mau disajikan sebagai sebuah laporan gitu.
...
- [done] aku barusan cek di laporan ada penggunaan diksi surel. semua diksi surel diganti dengan email ya.
  * **Selesai 2026-08-29.** `LaporanData::instansi()` "Surel distrans@..." → "Email distrans@...". Uji `memakai istilah email, bukan surel` diperluas ke rute `/laporan/*/dokumen` yang sebelumnya tidak ikut diperiksa. ui-spec §10.1 poin 1 tetap: komentar kode & dokumen `agents/` bebas memakai istilah mana pun.
- [done] Aku barusan cek ternyata laporan monografi itu cuma ambil dari Bab 2 ya? Coba buat juga yg dari Bab 3-Bab 5 untuk data yg sudah ada pada sistem kita. Yang tidak ada kita skip saja. Coba kamu list tiap Bab nya (selain Bab 2) kira2 data apa saja yg bakal kamu buat dari referensi laporan monografi di folder refs sesuai dengan data yg ada pada sistem ini. Nanti kita diskusikan.
  * **Selesai 2026-08-29 (Putaran 6).** Laporan Monografi SP kini memuat **Pendahuluan + Kependudukan + Sosial Ekonomi + Sosial Budaya** per SP, disaring dari tabel yang sudah ada. Judul **tanpa awalan "Bab X."** ("Keadaan Wilayah", bukan "Bab II. Keadaan Wilayah"). Struktur umur & mutasi penduduk **dikarang deterministik** (`DummyData::strukturUmurSp()` Σ=jiwa, `mutasiPendudukSp()` kumulatif tanpa perkawinan, `jiwaPerSp()` turunan dari `ringkasanDashboard()` — pengecualian sadar §19a). SP tanpa data → "belum ada data". Yang dilewati: tanaman perkebunan/peternakan/perikanan, harga bahan pokok, koperasi, layanan kesehatan/KB rinci, kelompok kesenian, organisasi desa, harta rumah tangga, jumlah guru/murid. Lihat bagian 1v.
- [done] Tambahkan mutasi penduduk rinci kecuali perkawinan. Begitupun juga di halaman data transmigran, tambahkan status kelahiran dan kematian. Tapi bagaimana dan kapan menambahkan status kelahiran atau kematian ya? Sebab itu kan statusnya semacam 'update' ketika datanya sudah ada.
  * **Selesai 2026-08-29 (Putaran 6).** Enum `StatusAnggotaKeluarga` {Aktif, Meninggal, Pindah} pada `anggota_keluarga` (+ `tanggal_peristiwa`, `keterangan_peristiwa`). **Membalik sebagian `rules.md` §9c**: anggota yang meninggal/pindah **tidak dihapus**, hanya ditandai, dan dikecualikan dari cacah jiwa / pengganti KK / rekap agama. Dicatat lewat tombol **"Catat Peristiwa"** per baris anggota (rute `transmigran.anggota.catat-peristiwa`). **Kepala keluarga TIDAK dapat field baru** — peristiwanya tetap lewat alur suksesi ("Ganti Kepala Keluarga"), sebab ia membawa rumah/lahan/poktan. **Kelahiran tidak ditandai di halaman** — hanya dihitung di Laporan Monografi dari `tanggal_lahir` vs `tahun_penempatan`. Mutasi laporan **kumulatif sejak penempatan**. Lihat bagian 1v.
- [done] Ini ada kesalahan fatal terkait relasi dan cara input data pada form alsintan dan saprotan di mana harusnya pada pengisian form-nya itu tidak diisi berulang kali untuk setiap poktan, sebab yg diisikan sama. Oh iya, untuk Alsintan tambahkan field form jenis alsinta juga ya sekalian yg bisa di-edit lewat data master. Contoh pada Alsintan: Untuk jenis alsintan traktor dengan nama traktor X, jumlah unit sekian, tahun pengadaan sekian, kondisi Y, dan sumber dana Z itu bisa dibagikan ke banyak Poktan. Begitupun juga dengan Saprotan. Nah, kalau diisi satu-satu kayak form sekarang ini, nanti admin bakal input redundan untuk tiap poktannya, padahal barangnya sama. Paham gak kamu? Coba kita diskusikan ya. Oh iya, nanti saat input Poktan, gunakan Searchable multi-select Poktan ya. Sekalian coba cari apakah ada masalah yg sama terkait relasi dan cara input data di sistem ini selain yg saya temukan di Alsintan dan Saprotan, karena ini fatal sekali.
  * **Selesai 2026-08-30 (Putaran 7, 10 commit, sembilan tahap).** Pola **INDUK + DISTRIBUSI**: baris induk mendeskripsikan bendanya, tabel distribusi satu baris per pihak penerima.
  * **Alsintan** → `alsintan` (pengadaan: `jenis_alsintan`, `nama_alat`, `jumlah_total`, `tahun_pengadaan`, `sumber_dana`) + `alsintan_distribusi` (poktan, jumlah, kondisi per poktan, penanda tangan, tanggal serah). Memilih beberapa poktan lewat **`<x-sim.pilih-cari-banyak>`** (searchable multi-select baru) → jumlah **dibagi rata otomatis**, dapat disunting. Pengadaan boleh "belum tersalurkan" (barang di gudang UPT). Daftar = satu baris per pengadaan.
  * **Saprotan** → sama. **`penanaman.saprotan_id` → `saprotan_distribusi_id`**: sisa benih kini dihitung per jatah poktan, bukan per pengadaan (dulu penanaman poktan A menggerus jatah poktan B). Aturan "sisa dihitung, tidak disimpan" (§7c.8) tetap utuh. Sekalian betulkan `satuan` (nama) → `satuan_id` (FK).
  * **`jenis_alsintan`** jadi data master (`JenisReferensi::JenisAlsintan`, disunting Admin di `/master/referensi/jenis_alsintan`). Dulu alsintan tak punya `jenis` sama sekali — "Jenis Alat" di laporan adalah `nama_alat` yang dipakai ulang.
  * **Audit menemukan 3 instans sama, semuanya diperbaiki:**
    (E) **`infrastruktur` lintas SP** — `KIOS SAPROTAN DESA` "melayani 3 SP" hanya diakui 1 SP, jadi SP tetangga jatuh ke "Perlu Penanganan" lewat aturan primer nol. **Skor SP yang salah**, bukan sekadar kurang rapi. `+satuan_permukiman_ids` + `infrastruktur_sp` cakupan; `PenilaianKondisiSp` baca cakupan.
    (F1) **`fasilitas_sp` lintas SP** — SMP Satu Atap, puskesmas pembantu, pasar desa. Pola sama (`fasilitas_sp_cakupan`).
    (F2) **`fasilitas_sp` / `inventaris_sp` `jumlah` > 1 dengan satu `kondisi`** — "dua dari tiga pos lapuk" lolos ke teks. `rincian_kondisi` (histogram kondisi → jumlah unit, Σ = jumlah). `kondisi` tetap penilaian umum petugas; `kondisiTerbaik()` baca rincian.
    (G) **`dokumen_lahan` satu dokumen banyak bidang** — satu HPL/SK mencakup ratusan bidang, dulu diketik ulang & berkasnya diunggah ulang per bidang. `dokumen_lahan` induk + `dokumen_lahan_bidang` (m2m). `dokumenLahan(?lahanId)` tanda tangan tetap.
    (H) **`hasil_panen.poktan_id`** salinan tanpa alasan → dicabut, diturunkan dari `penanaman.poktan_id`.
  * **Yang BUKAN bug** (keputusan sadar, tidak disentuh): `pengaduan_objek` (dicabut sengaja 2026-08-19), `penanaman.poktan_id` tunggal (laporan Polri per poktan), `transmigran.status_anggota_poktan`, `anggota_poktan`, `komoditas_poktan`, `riwayat_kepala_keluarga`, `rute_aksesibilitas_sp`.
  * Dokumen: `rules.md` §7b/§7c ditulis ulang + §7bc baru; `data-dictionary.md` §4.1/§4.2/§7.2/§8.3/§8.4/§10.1 + tabel batas #4/#5/#9/#33-35; `§11.37` betulkan `kualitas_panen` yang hanyut + tambah `jenis_alsintan`; `ui-spec.md` §6.0a sub-bagian multi-select. Lihat bagian 1w.
  * Verifikasi: pest 714, pint 31, `sim:tautan-statis` 223, seluruh suite peramban hijau. `uji-suksesi-kk` 14/5 = pra-ada (Edge headless).
  * **Belum diperiksa mata:** form repeater distribusi + form rincian kondisi di layar sungguhan, Ctrl+P laporan, rincian poktan dengan distribusi.

- [done] pada halaman detail transmigran di tab Anggota Keluarga, kenapa halaman utamanya ada horizontal slidebar-nya ya? Harusnya kan cukup di tabel anggota keluarga saja.
  * **Selesai 2026-08-30 (Revisi FE 1).** Memperbaiki containment horizontal overflow pada `resources/views/pages/transmigran/detail.blade.php`. Menambahkan `min-w-0 overflow-hidden` pada card wrapper dan `w-full max-w-full overflow-x-auto` pada container tabel 11 kolom. Scrollbar kini terisolasi rapi hanya pada tabel data keluarga tanpa memicu scrollbar horizontal pada halaman utama.
- [done] ubah jadi multistep form untuk data form halaman SP dan halaman Poktan (tambahkan form penambahan anggota poktan mirip kayak form data transmigran untuk penambahan anggota keluarga). Kira2 menurutmu ada lagi kah form data yg menurutmu sebaiknya dibuat multistep form karena banyak data yg harus di-input?
  * **Selesai 2026-08-30 (Revisi FE 2).** Diterapkan pada 3 form bermuatan data besar:
    1. **Form Data Lahan** (3 langkah): *Identitas & Pemilik* (SP + Pemilik KK + Jenis/Kategori Lahan), *Penggunaan & Lokasi* (Pola Tanam, Irigasi, Koordinat), *Legalitas & Berkas* (Status Kepemilikan, No Sertifikat/HPL, Upload Berkas).
    2. **Form SP** (4 langkah): *Identitas & Wilayah* (Nama, Kode, Tahun, Luas, Rencana KK, Desa & Kawasan), *Lokasi & Batas* (Koordinat, Letak Astronomis, Jarak Pusat Pemda, Batas Wilayah), *Keadaan Alam & Iklim* (SK Pencadangan, Pola Permukiman, Kesuburan & pH Tanah, Topografi/Kemiringan, Iklim & Suhu, Sumberdaya Air), *Aksesibilitas & Berkas* (Repeater Rute Aksesibilitas, Dokumen Penetapan SP, Catatan).
    3. **Form Kelompok Tani** (3 langkah): *Identitas Kelompok* (Satuan Permukiman paling atas, Nama Poktan, Tahun Berdiri, Alamat, Titik Koordinat), *Pengurus & Legalitas* (Ketua Kelompok [3 jalur: KK, Anggota Keluarga, Bukan Transmigran], Telepon, Email, Rekap Lahan, SK Pembentukan Poktan, Catatan), *Anggota Kelompok Tani* (Dynamic repeater penambahan anggota transmigran dengan pilihan jabatan master [Sekretaris, Bendahara, Anggota] dan catatan).
- [done] Di form Inventaris dan fasilitas SP, tambahkan field Jenis Inventaris dan jenis fasilitas berupa dropdown yang isinya bisa ditambah Admin via data master (isian wajib).
  * **Selesai 2026-08-30 (Revisi FE 3).** Menambahkan master referensi `JenisReferensi::JenisInventaris` (`'jenis_inventaris'`, kelompok Aset & Infrastruktur) dengan opsi baku *Peralatan Kantor*, *Elektronik & Mesin*, *Perabotan*, *Kendaraan Operasional*, *Peralatan Lainnya*. Disuplai ke view via `ViewServiceProvider`, ditambahkan kolom `jenis_inventaris` pada `DummyData::inventarisSp()`, ditaruh sebagai dropdown wajib di `pages/sp/form-inventaris.blade.php`, dan ditampilkan pada rincian `detail-inventaris.blade.php`. `jenis_fasilitas` sudah berupa master referensi wajib sejak sebelumnya.
- [done] Saat Tambah/ubah Data di semua form (kecuali pada halaman Transmigran, Alsintan, dan Saprotans), field yang punya tautan ke “parent”-nya default diisi paling awal/paling atas, baru mengisi yang lainnya. Beberapa di antaranya ada di Sub Menu Inventaris SP, Fasilitas SP, Infrastruktur SP, Rumah & Hunian, Data Lahan (rename dari “Daftar Lahan” menjadi “Data Lahan”), Kelompok Tani. Coba cek lagi menu yang lain, barangkali ada yg terlewat.
  * **Selesai 2026-08-30 (Revisi FE 4).** Seluruh form child menempatkan parent selector di urutan teratas:
    1. `pages/sp/form-inventaris.blade.php`: Satuan Permukiman & Jenis Inventaris di urutan paling atas.
    2. `pages/sp/form-fasilitas.blade.php`: Satuan Permukiman Lokasi di urutan paling atas.
    3. `pages/infrastruktur/form.blade.php`: Satuan Permukiman Lokasi di urutan paling atas.
    4. `pages/rumah/form.blade.php`: Wilayah-picker Satuan Permukiman di urutan paling atas.
    5. `pages/lahan/form.blade.php`: Wilayah-picker Satuan Permukiman & Pemilik KK di urutan paling atas. Menu & breadcrumb di-rename dari "Daftar Lahan" $\rightarrow$ "Data Lahan".
    6. `pages/poktan/form.blade.php`: Satuan Permukiman di urutan paling atas Bagian 1.
    7. Pengecualian form Transmigran, Alsintan, dan Saprotan dipertahankan sesuai arahan.
- [done] Di form Transmigran, wording "Status Tinggal" mungkin lebih baik diubah jadi "Status Tinggal Keluarga" agar meng-cover kondisi keluarga apabila ada yang rantau dll.
  * **Selesai 2026-08-30 (Revisi FE 5).** Mengubah label isian di `resources/views/pages/transmigran/form.blade.php` baris 278 dari "Status Tinggal" $\rightarrow$ "Status Tinggal Keluarga".
- [done] Upload KK sebaiknya dibuat wajib/mandatory, dan wording-nya ganti dari "Kartu Keluarga atau KTP" dengan "Kartu Keluarga (KK)".
  * **Selesai 2026-08-30 (Revisi FE 6).** Mengubah `x-sim.file-upload` KK di `resources/views/pages/transmigran/form.blade.php` menjadi `:wajib="true"`, label "Kartu Keluarga (KK)", dan nama dokumen "Kartu Keluarga".
- [done] Ubah wording di halaman Poktan yg berbunyi "Poktan di kawasan beserta ketua dan jumlah anggotanya." diganti dengan "Poktan di kawasan beserta ketua dan jumlah anggota transmigrannya."
  * **Selesai 2026-08-30 (Revisi FE 7).** Mengubah subjudul pada `resources/views/pages/poktan/index.blade.php` menjadi "Poktan di kawasan beserta ketua dan jumlah anggota transmigrannya."
- [done] Ini ada kesalahan fatal terkait bug horizontal scrollbar yg muncul di bawah pada halaman detail transmigran saat tab Anggota Keluarga diklik (bukti ss di `refs/bug tab anggota keluarga transmigran.jpg`).
  * **Selesai 2026-08-30.** Badan halaman `/transmigran/{id}?tab=keluarga` menggulir mendatar dan tergeser ke kanan hingga kolom ringkasan tertutup sidebar. **Dua sebab yang menumpuk:**
  * (a) Cangkang dua kolom halaman detail menyimpang dari pola baku `pages/poktan/detail.blade.php`: trek grid kanan `1fr` polos (minimum otomatisnya `min-content`) + kartu tab tanpa `min-w-0 overflow-hidden`. Diperbaiki di **seluruh** halaman detail — sebagian besar sudah dikerjakan sesi Gemini (`grid-cols-[20rem_minmax(0,1fr)]`, `min-w-0 overflow-hidden`, bilah tab `no-scrollbar`); di sini ditutup sisanya (`pages/master/penilaian-kondisi.blade.php`).
  * (b) **Sebab sebenarnya yg tersisa**: `<span class="sr-only">Aksi</span>` / `<caption class="sr-only">` (Tailwind `sr-only` = `position: absolute`) di dalam tabel yg menggulir mendatar. Tanpa `position: relative` pada wadah `.overflow-x-auto`, elemen sr-only itu memakai `<html>` sbg blok pengurung, lolos dari klip `overflow`, dan duduk di koordinat-x lebar tabel → `documentElement.scrollWidth` ikut melar. Perbaikan: `relative` pada wadah gulir tabel — komponen bersama `x-sim.tabel-ringkas` & `x-sim.data-table` (menular ke semua halaman daftar + tab detail), plus wadah mentah di `transmigran/detail`, `alsintan/detail`, `saprotan/detail`, `x-sim.modal-impor`, `x-sim.chart-card`.
...
* **Skema DDL Basis Data Final `database/data/schema.sql` (2026-09-01):**
  * **Catatan lokasi (2026-09-02):** berkas ini semula bernama `database/transmigrasi.sql`,
    lalu dipindah ke `database/data/schema.sql` agar berkumpul dengan berkas data
    lain yang bukan migration, yaitu `database/data/wilayah_indonesia.sql`. Nama lama
    masih tercatat di bawah sebagai riwayat; rujukan yang mengikat memakai nama baru.
  * **Konteks:**
    ...
    Lingkup: **hanya membuat 1 berkas baru** (kini `database/data/schema.sql`) — tanpa
...
- [done 2026-09-06] **Audit Putaran 16 oleh sesi berikutnya, lalu perbaikan MINOR-nya.**
  * Audit (3 sub-agen Explore + pembacaan langsung skema/migrasi): **0 BLOCKER, 0 MAYOR**. Ketujuh butir Gelombang 7 pada dasarnya akurat & didukung uji baru substansial. Sisa: 12 MINOR (konsistensi pola + celah uji + doc-rot). `BandingSkema` `GROUP_CONCAT` = perbaikan FK komposit, bukan pelonggaran.
  * **`/sp` & `/sp/{id}` kini disaring cakupan `Per SP`** (dulu bocorkan baris induk SP + Keadaan Wilayah/Monografi ke petugas SP lain; data per-warga sudah aman). Docblock `SatuanPermukiman` yang keliru ("global scope menempel") diperbaiki: model ini sengaja TAK ber-global-scope (`/kawasan` kawasan-lebar), penyaringan lewat `scopeTerlihatOlehPengguna()`. `SpRuntimeTest` disesuaikan (404 untuk SP tak ditugaskan) + uji SP yang ditugaskan tetap terbuka.
  * **`hasil_panen` unik kini ber-gate `deleted_at`** (`uq_hasil_panen_penanaman_aktif` atas kolom turunan `penanaman_aktif_id`, pola sama `anggota_poktan`/`pending_email_changes`) lewat migration terpisah `2026_09_06_130000`. Panen boleh dicatat ULANG setelah catatan salah di-soft delete -- sebelumnya terblokir permanen tanpa alur restore. `HasilPanenController::validasi()` & `Penanaman::hasilPanen()` tak lagi `withTrashed()`. `HasilPanenTest`/`Domain8ProduksiTest` dibalik.
  * **Penjaga hapus lintas-SP eksplisit** ditambah di `Rumah/Lahan/Transmigran::hapus` (`CakupanDataSp::pastikanDapatDitulis`), belt-and-suspenders sejalan Poktan/Alsintan/dll + 3 uji baru. `Alsintan/Saprotan::hapus` tetap 403 (bukan 404) dgn komentar: pengadaan terlihat semua role, yang ditolak tindakannya.
  * **Bug frontend dari laporan pemilik proyek:** `rumah/form.blade.php` mengoper `:sp-terkunci="statusHunian === 'Dihuni' && penghuniId !== ''"` ke `wilayah-picker`, TETAPI `penghuniId` tak pernah dideklarasikan di `x-data` form -> Alpine hanya mencatat warning dan dropdown SP tetap dapat diubah meski KK sudah dipilih. (`lahan/form.blade.php` benar: `pemilikId` dideklarasikan.) Diperbaiki: `penghuniId` ditambah ke `x-data` + di-set di `gantiPenghuni()`. Uji `RumahTest` "mengunci pilihan SP" diperkuat -- kini juga menuntut `penghuniId` dideklarasikan, bukan hanya string `:disabled` ada. Seluruh form lain yg pakai pola kunci-SP / quick-fill (`lahan`, `poktan`, `poktan-anggota`, `penanaman`, `saprotan`, `panen`, master, `sp/form-kawasan`) disisir manual -> hanya `rumah` yang cacat.
  * **Celah uji ditutup:** throttle POST `email-change.confirm`, TOCTOU email direbut, actor != confirmed user tak ter-logout, permintaan email terbengkalai, formula pada CSV (bukan hanya XLSX), email HTML anti-"surel".
  * **Doc & kode mati:** `ViewServiceProvider::memakaiDataContoh()` di-inline (bukan dihapus, DRY); komentar rute usang ("masih DummyData") diperbarui; `ui-spec.md` §10.1 diberi pemetaan modul→menu/halaman/bagian/fitur; komentar portabilitas SQLite ≥ 3.31 di 3 migration generated-column; BOM UTF-8 dicabut dari `notes.md`.
  * **Migration idempoten `2026_09_06_140000`**: menyelaraskan FK komposit `lahan` + `uq_transmigran_id_sp` bagi DB yang bermigrasi SEBELUM Putaran 16 menyunting migration Tahap-3. No-op pada `migrate:fresh` (paritas `schema.sql` tetap).
  * **Sengaja TIDAK dikerjakan:** rename migration `_000000` kembar (menyunting migration yang sudah tercatat = re-run pada DB non-fresh, risikonya melebihi manfaat kerapian); memo `nilaiRujukan()` (risiko data rujukan basi di proses uji melebihi hemat kueri per-render yang minor); menyembunyikan nama kolom di Catatan Log (identifier field, bukan nilai; konsisten dgn `/audit-log`).
  * Verifikasi: pint bersih, `sim:banding-skema --lengkap` **NOL SELISIH**, Feature **764 PASS / 7.587 assertions**, Database **583 PASS / 2.632 assertions**.
