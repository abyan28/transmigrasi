...
it('menyediakan seluruh jenis daftar pilihan pada data contoh', function () {
    ...
    foreach (JenisDaftarPilihan::cases() as $jenis) {
        expect(DummyData::daftarPilihan($jenis))
...
it('menyediakan jenis alsintan sebagai data master yang dapat disunting admin', function () {
    ...
    expect(JenisDaftarPilihan::JenisAlsintan->value)->toBe('jenis_alsintan')
        ->and(JenisDaftarPilihan::JenisAlsintan->kelompok())->toBe(KelompokDaftarPilihan::AsetInfrastruktur)
        ->and(JenisDaftarPilihan::JenisAlsintan->berskor())->toBeFalse()
        ->and(JenisDaftarPilihan::JenisAlsintan->berjenjang())->toBeFalse()
        ->and(JenisDaftarPilihan::JenisAlsintan->dirujukParameter())->toBeFalse();

    $nilai = array_column(DummyData::daftarPilihan(JenisDaftarPilihan::JenisAlsintan), 'nilai');
    expect($nilai)->toContain('Traktor Roda Dua')->toContain('Pompa Air')->toContain('Lainnya');

    // Halamannya sendiri di /master/daftar-pilihan/jenis_alsintan.
    $this->get(route('daftar-pilihan.jenis', ['jenis' => 'jenis_alsintan']))->assertOk();

    // Id infrastruktur/fasilitas tetap di tempatnya: tak ada penomoran ulang.
    $idInfraTerakhir = max(array_column(DummyData::daftarPilihan(JenisDaftarPilihan::JenisInfrastruktur), 'id_daftar_pilihan'));
    $idAlsintanPertama = min(array_column(DummyData::daftarPilihan(JenisDaftarPilihan::JenisAlsintan), 'id_daftar_pilihan'));
...
it('memisahkan nilai aktif dari nilai yang sudah dinonaktifkan', function () {
    ...
    $semua = DummyData::daftarPilihan();
    $aktif = DummyData::daftarPilihan(null, true);
    ...
    foreach ($nonaktif as $baris) {
        $opsi = DummyData::opsiDaftarPilihan(JenisDaftarPilihan::from($baris['jenis']));
...
it('memberi nilai skor hanya pada jenis yang benar-benar dipakai menghitung', function () {
    ...
    expect(JenisDaftarPilihan::Kondisi->berskor())->toBeTrue();
    expect(JenisDaftarPilihan::KondisiRumah->berskor())->toBeFalse();

    foreach (DummyData::daftarPilihan() as $baris) {
        $jenis = JenisDaftarPilihan::from($baris['jenis']);
...
it('menyortir prioritas pengaduan memakai urutan, bukan abjad', function () {
    ...
    expect(JenisDaftarPilihan::PrioritasPengaduan->berjenjang())->toBeTrue();
    expect(JenisDaftarPilihan::SumberDana->berjenjang())->toBeFalse();

    $prioritas = array_column(DummyData::daftarPilihan(JenisDaftarPilihan::PrioritasPengaduan), 'nilai');
...
it('tidak lagi memakai enum untuk daftar yang sudah menjadi data master', function () {
    ...
    $enumMaster = [
        ...
        'TipeKomoditas', 'PrioritasPengaduan',
...
it('menampilkan seluruh daftar sebagai kartu, bukan tab yang tersembunyi', function () {
    ...
    $isi = $this->get(route('master.daftar-pilihan'))->assertOk()->getContent();
    ...
    foreach (JenisDaftarPilihan::cases() as $jenis) {
        ...
        expect($isi)->toContain(route('daftar-pilihan.jenis', ['jenis' => $jenis->value]));
    }
...
it('mengelompokkan setiap jenis tanpa menyisakan satu pun', function () {
    ...
    sort($terkumpul);
    $seluruhnya = array_column(JenisDaftarPilihan::cases(), 'value');
    sort($seluruhnya);
...
it('memberi setiap daftar halamannya sendiri', function () {
    foreach (JenisDaftarPilihan::cases() as $jenis) {
        $isi = $this->get(route('daftar-pilihan.jenis', ['jenis' => $jenis->value]))
            ->assertOk()
            ->getContent();
...
it('mengalihkan alamat tab lama ke halaman daftarnya', function () {
    ...
    $this->get('/master/daftar-pilihan?tab=kondisi')
        ->assertRedirect(route('daftar-pilihan.jenis', ['jenis' => 'kondisi']));

    // Tab karangan tidak mengalihkan, hanya menampilkan indeks.
    $this->get('/master/daftar-pilihan?tab=karangan')->assertOk();
});
it('tidak menyediakan penghapusan nilai daftar pilihan', function () {
    ...
    expect(Route::has('daftar-pilihan.hapus'))->toBeFalse();

    foreach ([1, 2, 3, 4] as $idRole) {
        $izin = DummyData::izinRole($idRole)['daftar_pilihan'] ?? [];

        expect($izin)->not->toContain('hapus');
    }

    expect(DummyData::izinRole(1)['daftar_pilihan'])->toBe(['lihat', 'tambah', 'ubah']);
    expect(DummyData::izinRole(2)['daftar_pilihan'])->toBe(['lihat', 'tambah', 'ubah']);
    expect(DummyData::izinRole(3)['daftar_pilihan'])->toBe(['lihat']);
    expect(DummyData::izinRole(4)['daftar_pilihan'])->toBe(['lihat']);
});
...
it('merujuk jenis infrastruktur dan fasilitas lewat id, bukan teks', function () {
    ...
    foreach (PenilaianKondisiSp::parameter() as $par) {
        expect($par)->toHaveKey('daftar_pilihan_id');
        expect($par)->not->toHaveKey('jenis_rujukan');
        expect($par['daftar_pilihan_id'])->toBeInt();

        // Idnya harus benar-benar menunjuk baris yang ada, dan pada jenis yang
        // sesuai sumbernya.
        $jenisDituju = $par['sumber'] === 'Fasilitas'
            ? JenisDaftarPilihan::JenisFasilitas
            : JenisDaftarPilihan::JenisInfrastruktur;

        $baris = array_values(array_filter(
            DummyData::daftarPilihan($jenisDituju),
            fn ($b) => $b['id_daftar_pilihan'] === $par['daftar_pilihan_id']
        ));
...
it('tetap menilai sama setelah ejaan jenis rujukan diganti', function () {
    ...
    // dengan satu nilai daftar pilihan yang ejaannya diganti berikut aset yang
    // memakainya. Hasilnya wajib sama, sebab rujukannya memakai id.
    $sebelum = PenilaianKondisiSp::nilai(1);

    $idAir = DummyData::daftarPilihanId(JenisDaftarPilihan::JenisInfrastruktur, 'Air');

    expect($idAir)->not->toBeNull();

    // Aset dengan ejaan baru, seolah Admin sudah menyunting nilai daftar pilihannya.
...
it('melewati parameter yang rujukannya hilang, bukan menilainya nol', function () {
    ...
    // karena satu baris daftar pilihan hilang, dan pada parameter primer itu
...
it('membedakan bidang kosong yang bermakna dari bidang yang terlewat diisi', function () {
    ...
    $kategori = DummyData::daftarPilihan(JenisDaftarPilihan::KategoriPengaduan);
    ...
    foreach ($kategori as $baris) {
        ...
        expect(DummyData::daftarPilihanNilai($baris['bidang_id']))
    ...
    expect($this->get(route('daftar-pilihan.jenis', ['jenis' => JenisDaftarPilihan::KategoriPengaduan->value]))->getContent())
...
it('memakai daftar lengkap pada filter, bukan hanya nilai aktif', function () {
    ...
    expect(DummyData::opsiFilterDaftarPilihan(JenisDaftarPilihan::SumberDana))
        ->toHaveKey('Lembaga Swadaya Masyarakat');

    expect(DummyData::opsiDaftarPilihan(JenisDaftarPilihan::SumberDana))
    ...
    foreach (File::allFiles(resource_path('views')) as $berkas) {
        ...
        foreach ($baris as $nomor => $isi) {
            if (! str_contains($isi, 'opsiDaftarPilihan(')) {
                continue;
            }
